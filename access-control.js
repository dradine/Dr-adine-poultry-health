...
window.AdineAccess={
 ...
 async farmIds(){const {data,error}=await supabaseClient.rpc('get_my_farm_access'); if(error){console.error('get_my_farm_access:',error);return []} return (data||[]).filter(x=>x.connection_status==='active').map(x=>x.farm_id)},
 async canAccessFarm(id){const a=await this.current();if(!a)return false;if(['owner','admin'].includes(String(a.profile.role||'').toLowerCase()))return true;if(String(a.profile.user_type||'').match(/farm_operator|farm_manager|company_manager/)) {const {data}=await supabaseClient.from('farms').select('id').eq('id',id).eq('owner_id',a.user.id).maybeSingle();return !!data}const ids=await this.farmIds();return ids.includes(id)},
