...
(() => {
  ...
  function db() { return window.supabaseClient || window.supabaseClient?.from ? window.supabaseClient : window.supabase; }
  ...
  function ensureStyle() {
    ...
    s.textContent=`
      ...
      @media(max-width:600px){.audit-change{grid-template-columns:1fr}.audit-modal{padding:13px}}
  ...
  function modal(title, recordId) {
    ...
    back.innerHTML=`<div class="audit-modal" role="dialog" aria-modal="true" aria-labelledby="auditTitle"><div class="audit-head"><h3 id="auditTitle">${esc(title)}</h3><button class="audit-close" type="button">بستن</button></div><div class="audit-record">شناسه رکورد: ${esc(recordId)}</div><div id="auditBody" class="audit-list"><div class="audit-loading">در حال دریافت تاریخچه...</div></div></div>`;
  ...
  function renderRows(body, rows) {
    if(!rows.length){body.innerHTML='<div class="audit-empty">برای این رکورد هنوز تاریخچه‌ای ثبت نشده است.</div>';return;}
    body.innerHTML=rows.map(r=>{
      ...
      return `<div class="audit-row"><div class="audit-row-top"><span class="audit-badge ${op.toLowerCase()}">${esc(ACTION_FA[op]||op||'رویداد')}</span><b>${esc(who)}</b><span class="audit-badge">${esc(role)}</span><span class="audit-meta">${esc(dateFa(r.occurred_at))}</span></div>${changeHtml}</div>`;
    }).join('');
  }

  async function showHistory(kind, id, title) {
    const body=modal(title,id); const client=db();
    if(!client){body.innerHTML='<div class="audit-empty">اتصال سامانه آماده نیست.</div>';return;}
    const table=TABLE_MAP[kind];
    let q=client.from('health_activity_log').select('*').eq('record_id',String(id)).order('occurred_at',{ascending:false}).limit(100);
    const fid=flockId(); if(fid) q=q.eq('flock_id',fid);
    if(table) q=q.eq('table_name',table);
    const {data,error}=await q;
    if(error){console.error('Audit history:',error);body.innerHTML='<div class="audit-empty">تاریخچه قابل دریافت نیست. سطح دسترسی یا اتصال سامانه را بررسی کنید.</div>';return;}
  ...
  function scan() {
    // Health page: existing action buttons expose data-edit/data-delete.
    ...
    // Mortality page: existing inline actions contain the event id.
  ...
  window.ADInehAuditHistory={showHistory};
})();
