...
(function(global){
  ...
  function captureModel(){
    ...
    api.build=function(){const model=original.apply(this,arguments);if(model?.domain==='broiler')global.__adineBroilerComprehensiveModel=model;return model};
  ...
  function ensureCumulativeGainChart(){
    const canvas=$('cr2CumulativeGain'),model=global.__adineBroilerComprehensiveModel;
    ...
    cumulativeGainChart=new Chart(canvas,{type:'line',data:{labels,datasets:[
      {label:'افزایش وزن تجمعی واقعی',data:model.rows.map(r=>n(r.cumulativeWeightGain)),borderWidth:2,tension:.25,pointRadius:3,fill:false,spanGaps:true},
      {label:'مرجع رسمی افزایش وزن تجمعی',data:model.rows.map(r=>n(r.standardCumulativeWeightGain)),borderWidth:2,tension:.25,pointRadius:2,fill:false,spanGaps:true,borderDash:[6,4]}
  ...
  function injectChart(){
    ...
    const section=document.createElement('section');section.className='cr2-chart-card cr2-cumulative-gain-fix';
