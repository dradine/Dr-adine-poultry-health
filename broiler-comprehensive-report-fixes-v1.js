/* ADINE — BROILER COMPREHENSIVE REPORT FIXES V2
...
(function(global){
  ...
  function injectChart(){
    if($('cr2CumulativeFcr')){patchTrendUnit();ensureCumulativeFcrChart();return}
    const fcr=$('cr2Fcr');if(!fcr)return;
    const host=fcr.closest('.cr2-chart-card')||fcr.closest('.chart-card')||fcr.parentElement?.parentElement||fcr.parentElement;if(!host?.parentNode)return;
    ...
    host.parentNode.insertBefore(section,host.nextSibling);patchTrendUnit();ensureCumulativeFcrChart();
  }
  ...
  global.AdineBroilerComprehensiveReportFixesV1={version:'BROILER-COMPREHENSIVE-REPORT-FIXES-V2'};
