...
(function(global){
  ...
  function getModel(){return global.__adineBroilerComprehensiveModel||null}
  ...
  function ensureCumulativeGainChart(){
    const canvas=$('cr2CumulativeGain');
    ...
    const actual=model.rows.map(r=>n(r.cumulativeWeightGain));
    const reference=model.rows.map(r=>n(r.standardCumulativeWeightGain));
    cumulativeGainChart=new Chart(canvas,{type:'line',data:{labels,datasets:[
      {label:'افزایش وزن تجمعی واقعی',data:actual,borderWidth:2,tension:.25,pointRadius:3,fill:false,spanGaps:true},
      {label:'مرجع رسمی افزایش وزن تجمعی',data:reference,borderWidth:2,tension:.25,pointRadius:2,fill:false,spanGaps:true,borderDash:[6,4]}
  ...
  function injectChart(){
    ...
    const gain=$('cr2Gain');
    if(!gain)return;
    const host=gain.closest('.cr2-chart-card')||gain.closest('.chart-card')||gain.parentElement?.parentElement||gain.parentElement;
    if(!host||!host.parentNode)return;
    const section=document.createElement('section');
    section.className='cr2-chart-card cr2-cumulative-gain-fix';
    section.innerHTML='<div class="cr2-chart-head"><div><div class="eyebrow">رشد تجمعی</div><h3>افزایش وزن تجمعی</h3></div><span class="scope-badge">واقعی در برابر مرجع رسمی</span></div><div class="cr2-chart-wrap"><canvas id="cr2CumulativeGain"></canvas></div>';
    host.parentNode.insertBefore(section,host.nextSibling);
    patchTrendUnit();
    ensureCumulativeGainChart();
  }
  function schedule(){setTimeout(()=>{patchTrendUnit();injectChart()},80)}
  const observer=new MutationObserver(schedule);
  function init(){schedule();observer.observe(document.body,{childList:true,subtree:true});}
