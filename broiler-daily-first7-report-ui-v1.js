...
const ENGINE=()=>window.ADINE_BROILER_FIRST7_REPORT_ENGINE_V1;const $=id=>document.getElementById(id);let busy=false;
function sanitize(m){let lastBw=null;m.days.forEach(d=>{if(d.bw!=null&&d.bw<=0)d.bw=null;if(d.bw==null){d.gain=null;d.growthPct=null;d.dev=null}else if(lastBw!=null){d.gain=d.bw-lastBw;d.growthPct=lastBw>0?d.gain/lastBw*100:null;d.dev=d.target!=null?(d.bw-d.target)/d.target*100:null}else{d.gain=null;d.growthPct=null;d.dev=d.target!=null?(d.bw-d.target)/d.target*100:null}if(d.bw!=null)lastBw=d.bw});return m}
async function showDaily(){const root=$('root'),selector=$('weeklyWeekSelectorSlot');if(!root)return;if(selector)selector.style.display='none';root.style.display='block';if(busy)return;busy=true;root.innerHTML='<div class="f7-empty" style="padding:48px;text-align:center">در حال ساخت گزارش روزانه هفت روز اول…</div>';try{const e=ENGINE();if(!e)throw new Error('موتور گزارش روزانه بارگذاری نشده است.');const m=sanitize(await e.load());root.innerHTML=e.render(m)}catch(err){console.error(err);root.innerHTML=`<div class="error">خطا در بارگذاری گزارش روزانه: ${String(err?.message||err)}</div>`}finally{busy=false}}
