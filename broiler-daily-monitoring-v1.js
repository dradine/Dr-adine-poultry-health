/* ADINE — BROILER DAILY MONITORING V1
   Independent from weekly engines/calculations.
*/
(function(){
  'use strict';
  const S=window.ADINE_BROILER_DAILY_STANDARDS_V1||{};
  let flock=null, user=null, selectedDate=null, currentRecord=null, records=[];
  ...
  const fmt=v=>v==null||v===''?'—':Number(v).toLocaleString('fa-IR',{maximumFractionDigits:2});
  ...
  function fillAuto(){
    ...
    set('doaPercent',base&&a===1?((doa/base)*100).toFixed(3):a===1? '—':(initialCount()?((doa/initialCount())*100).toFixed(3):'—'));
