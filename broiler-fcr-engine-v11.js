/* ADINE POULTRY HEALTH — BROILER FCR ENGINE V13.1 */
(function(global){'use strict';
const VERSION='BROILER-FCR-V13.1';
...
function canonical(records,flock){
  ...
  const iw=n(flock?.initial_average_weight_g??flock?.initialAverageWeightG);
  const ib=n(flock?.initial_bird_count??flock?.initialBirdCount);
  let prev=null,cumFeed=0;
  return sorted.map((r,index)=>{
    const weight=n(r.average_weight_g??r.averageWeight);
    const live=n(r.live_birds??r.liveBirds);
    const feed=n(r.feed_total_kg??r.feedTotalKg??r.feed);
    const openingLive=index===0?ib:n(prev?.live_birds??prev?.liveBirds);
    const openingWeight=index===0?iw:n(prev?.average_weight_g??prev?.averageWeight);
    let biomassGain=null,weekly=null;
    if(live>0&&weight>0&&openingLive>0&&openingWeight!==null){
      biomassGain=(live*weight-openingLive*openingWeight)/1000;
      if(feed>0&&biomassGain>0)weekly=feed/biomassGain;
    }
    if(feed!==null&&feed>=0)cumFeed+=feed;
    const cumulativeGain=(live>0&&weight>0&&ib>0&&iw!==null)?(live*weight-ib*iw)/1000:null;
    let cumulative=null;
    if(index===0)cumulative=weekly;
    else if(cumFeed>0&&cumulativeGain>0)cumulative=cumFeed/cumulativeGain;
    const out={...r,ageDays:n(r.ageDays??r.age_days),weeklyFcr:weekly==null?null:Number(weekly.toFixed(4)),cumulativeFcr:cumulative==null?null:Number(cumulative.toFixed(4)),fcr:weekly==null?null:Number(weekly.toFixed(4)),calculationVersion:VERSION};
    prev=r;
    return out;
  });
}
