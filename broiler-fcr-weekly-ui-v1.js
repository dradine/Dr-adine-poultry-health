/* ADINE POULTRY HEALTH — BROILER FCR WEEKLY UI BRIDGE V1.4
...
(function(global){'use strict';
  const VERSION='BROILER-FCR-WEEKLY-UI-V1.4';
  const num=v=>{
  ...
  function meanFromWeightInputs(){
    const values=Array.from(document.querySelectorAll('.bird-weight')).map(x=>num(x?.value)).filter(x=>x!==null&&x>0);
  ...
  function canonicalFallback(i,prev,f){
    const engine=global.AdineBroilerFCR;
    if(!engine?.canonical)return null;
    ...
    const rows=[{age_days:firstNumber(prev?.age_days,prev?.ageDays,0),live_birds:openBirds,average_weight_g:openWeight,feed_total_kg:0},{age_days:firstNumber(i.age,i.week*7,7),live_birds:i.live,average_weight_g:i.weight,feed_total_kg:i.feed}];
    const out=engine.canonical(rows,f);return out.length?out[out.length-1].weeklyFcr:null;
  }
  function weeklyFcr(result){
    ...
    const input={feedKg:i.feed,openBirds,openWeight,openAgeDays,closeBirds:i.live,closeWeight:i.weight,closeAgeDays:i.age};
    let value=null;
    try{value=p.broilerWeeklyFCR(input)}catch(e){console.warn('Broiler FCR adapter:',e)}
    return value!=null?value:canonicalFallback(i,prev,f);
  }
  function cumulativeFcr(result){
    ...
    try{return p.broilerCumulativeFCR([...prior,current],f)}catch(e){return null}
  }
  ...
  function patchRender(){
    ...
    wrapped.__adineBroilerFcrV14=true;global.renderResults=wrapped;return true;
  }
  function patchCalculateWeekly(){
    if(typeof global.calculateWeekly!=='function'||global.calculateWeekly.__adineBroilerFcrV14)return false;
    const original=global.calculateWeekly;
    function wrappedCalculateWeekly(){const out=original.apply(this,arguments);setTimeout(()=>{try{showFcr(out)}catch(e){}},0);setTimeout(()=>{try{showFcr(out)}catch(e){}},100);setTimeout(()=>{try{showFcr(out)}catch(e){}},500);return out}
    wrappedCalculateWeekly.__adineBroilerFcrV14=true;global.calculateWeekly=wrappedCalculateWeekly;return true;
  }
  function observeResults(){
    if(global.__adineBroilerFcrResultsObserver||typeof MutationObserver==='undefined')return;
    const root=document.getElementById('results');if(!root)return;
    let scheduled=false;
    const run=()=>{if(scheduled)return;scheduled=true;setTimeout(()=>{scheduled=false;try{if(isBroiler()&&root.children.length)showFcr(null)}catch(e){}},30)};
    new MutationObserver(run).observe(root,{childList:true,subtree:true});
    global.__adineBroilerFcrResultsObserver=true;
  }
  function patchCalculateButton(){
    if(document.documentElement?.dataset?.adineBroilerFcrCalculateHook==='2')return true;
    if(!document.documentElement)return false;
    document.documentElement.dataset.adineBroilerFcrCalculateHook='2';
    document.addEventListener('click',function(event){
      ...
      if(!/calculateWeekly\s*\(|محاسبه پایش|محاسبه/.test(inline+' '+text))return;
      [0,50,150,400,800].forEach(ms=>setTimeout(()=>{try{if(isBroiler())showFcr(null)}catch(e){}},ms));
    },true);return true;
  }
  function patchSave(){
    if(typeof global.saveWeeklyRecord!=='function'||global.saveWeeklyRecord.__adineBroilerFcrV14)return false;
    ...
    wrappedSave.__adineBroilerFcrV14=true;global.saveWeeklyRecord=wrappedSave;return true;
  }
  function start(){
    let i=0;const timer=setInterval(()=>{installBroilerCompatibility();patchRender();patchCalculateWeekly();patchCalculateButton();patchSave();observeResults();if(++i>240)clearInterval(timer)},250);
    installBroilerCompatibility();patchRender();patchCalculateWeekly();patchCalculateButton();patchSave();observeResults();
  }
