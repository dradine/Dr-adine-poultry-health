/* ADINE — comparison result labels
   ...
   Strain is taken ONLY from the flock strain field; genetics/company is never used.
   Farm, house and placement date remain excluded from result labels.
   Chart labels use the exact same flock label.
...
(function(){
  ...
  function getLabel(i){
    ...
    const flockName=window.__adineComparisonFlocks?.[i]?.flock_name;
    if(flockName)name=flockName.trim();

    const strain=window.__adineComparisonFlocks?.[i]?.strain?.trim()||'';
