...
function multivariateAnalysis(rows,s){
 ...
 const out=[]; const push=(code,severity,title,text,ev,causes,checks)=>out.push({code,severity,title,text,evidence:ev,possibleCauses:causes,checks,confidence:evidenceLevel(rows,ev),type:'multivariate'});
 const bad=k=>['watch','critical'].includes(s[k]?.official?.status), good=k=>['good','excellent'].includes(s[k]?.official?.status);
 const wors=k=>s[k]?.trend?.direction==='worsening', impr=k=>s[k]?.trend?.direction==='improving';
 if(bad('weight')&&bad('fcr')&& (bad('cv')||bad('mortality'))) push('growth_efficiency_distribution','high','افت همزمان رشد، کارایی و کیفیت گله','وزن از مرجع عقب است و FCR نیز نامطلوب است؛ همراهی CV یا تلفات، الگو را از یک انحراف منفرد به یک مسئله چندبعدی عملکردی تبدیل می‌کند.',['weight','fcr','cv'],'خوراک و آب، شرایط محیطی، سلامت روده/بیماری، کیفیت خوراک، مدیریت دسترسی به خوراک و آب','مصرف دان و آب، کیفیت خوراک، دما/RH، بستر، علائم بالینی و روند تلفات');
 if(good('weight')&&bad('fcr')&&good('cv')) push('growth_efficiency_tradeoff','watch','رشد حفظ شده با افت کارایی خوراک','وزن و یکنواختی مناسب‌اند اما FCR نامطلوب است؛ بنابراین رشد فعلی با efficiency پایین‌تری حاصل شده و باید مصرف واقعی خوراک و روند FCR تجمعی بررسی شود.',['weight','fcr','cv'],'افزایش مصرف بدون رشد متناسب، کیفیت خوراک، شرایط محیطی، ترکیب جیره یا تغییرات مدیریتی','مصرف خوراک، FCR تجمعی، ADG، آب/خوراک و شرایط محیطی');
 if(bad('weight')&&good('fcr')&&impr('adg')) push('weight_recovery_efficiency_ok','watch','فاصله وزن وجود دارد اما سیگنال بازیابی رشد دیده می‌شود','وزن هنوز عقب است ولی ADG در مسیر بهبود و FCR فعلاً مناسب است؛ این الگو با شروع recovery سازگار است و نباید صرفاً از روی وزن فعلی هشدار شدید صادر شود.',['weight','fcr','adg'],'تاخیر زمانی بین اصلاح رشد و رسیدن وزن تجمعی به هدف','ادامه روند ADG و وزن در ارزیابی بعدی');
 if(bad('cv')&&bad('u10')&&bad('u15')) push('distribution_broadening','high','گسترش پراکندگی وزن در چند سطح','CV بالا و افت همزمان U10 و U15 نشان می‌دهد مسئله فقط میانگین وزن نیست و توزیع گله نیز نامطلوب‌تر شده است.',['cv','u10','u15'],'شروع نامتوازن، دسترسی نامتوازن به دان/آب، تراکم، محیط، بیماری یا تفاوت مصرف','توزیع خام وزن، نقاط انتهایی، feeder/drinker access، محیط و سلامت');
 if(bad('mortality')&&bad('fcr')&&wors('mortality')&&wors('fcr')) push('survival_efficiency','high','فشار همزمان بر بقا و کارایی','تلفات و FCR هر دو در جهت نامطلوب حرکت کرده‌اند؛ این هم‌زمانی مهم است اما علت اختصاصی از داده عملکردی به تنهایی قابل تعیین نیست.',['mortality','fcr'],'بیماری، استرس محیطی، کیفیت آب/خوراک، مشکلات مدیریتی و افزایش حذف','تفکیک تلفات روزانه/تجمعی، necropsy، آب، خوراک، محیط و رخدادهای اخیر');
 if(wors('weight')&&wors('adg')&&wors('fcr')) push('trajectory_decline','high','مسیر رشد و کارایی همزمان در حال تضعیف است','وزن، سرعت رشد و FCR همزمان در جهت نامطلوب حرکت می‌کنند؛ اگر این مسیر ادامه یابد فاصله وزن و هزینه تبدیل خوراک می‌تواند بیشتر شود.',['weight','adg','fcr'],'هر عامل مشترکی که مصرف یا جذب/استفاده از مواد مغذی را مختل کند؛ از جمله خوراک، آب، محیط و سلامت','ارزیابی تغییرات آخرین دو تا سه دوره و داده‌های مصرف');
 if(impr('weight')&&impr('adg')&&impr('fcr')) push('coherent_recovery','positive','بهبود هم‌جهت رشد و کارایی','وزن و ADG بهبود یافته و FCR نیز در جهت مطلوب حرکت کرده است؛ این هم‌جهتی از یک recovery چندشاخصی حمایت می‌کند.',['weight','adg','fcr'],'بهبود شرایط مدیریت/خوراک/محیط یا بازگشت گله از یک افت قبلی','تداوم این الگو در ارزیابی بعدی');
 if(bad('u10')&&good('weight')) push('mean_masks_distribution','watch','میانگین وزن مناسب است اما توزیع نیازمند توجه است','وزن متوسط مناسب است ولی U10 نامطلوب است؛ بنابراین میانگین به تنهایی کیفیت کل گله را توصیف نمی‌کند.',['weight','u10','cv'],'پراکندگی وزن، گروه‌های عقب‌مانده/جلوتر و ناهمگونی مصرف','نمونه خام وزن و توزیع نسبت به مرجع');
 if(wors('cumulativeFcr')&&impr('fcr')) push('weekly_cumulative_divergence','watch','بهبود هفتگی هنوز در FCR تجمعی منعکس نشده است','FCR اخیر در جهت مطلوب حرکت کرده ولی FCR تجمعی هنوز بدتر می‌شود؛ این می‌تواند اثر تاخیری عملکرد هفته‌های قبل باشد و نباید تناقض واقعی تلقی شود.',['fcr','cumulativeFcr'],'اثر حافظه‌ای FCR تجمعی و وزن‌دهی دوره‌های قبلی','روند حداقل سه ارزیابی FCR هفتگی و تجمعی');
 if(bad('weight')&&bad('adg')&&good('fcr')) push('slow_growth_not_feed_efficiency','watch','مشکل اصلی در حال حاضر رشد است نه الزاماً تبدیل خوراک','FCR فعلاً مناسب است اما وزن و ADG عقب‌اند؛ تمرکز باید روی علت کندی رشد و کافی‌بودن مصرف باشد.',['weight','adg','fcr'],'مصرف ناکافی، دسترسی به دان/آب، محیط، سلامت یا سن/برنامه رشد','feed intake، water intake، محیط و وضعیت سلامت');
 return out;
}
function differentialAnalysis(rows,s,patterns){
 const map={
  growth_efficiency_distribution:[['خوراک/آب','بررسی مصرف واقعی، کیفیت و دسترسی'],['محیط','بررسی دما، RH، تهویه، بستر و آمونیاک'],['سلامت','بررسی علائم، necropsy و روند تلفات'],['مدیریت','بررسی تراکم و دسترسی feeder/drinker']],
  growth_efficiency_tradeoff:[['خوراک','بررسی مصرف و کیفیت خوراک'],['رشد','مقایسه ADG با وزن و FCR تجمعی'],['محیط','بررسی گرما و شرایط سالن']],
  distribution_broadening:[['دسترسی','feeder/drinker space و توزیع دسترسی'],['محیط','یکنواختی دما/تهویه'],['سلامت','بیماری‌های با اثر بر رشد یکنواخت']],
  survival_efficiency:[['سلامت','تفکیک علت تلفات و necropsy'],['محیط','دما/RH/آمونیاک/بستر'],['خوراک و آب','کیفیت و مصرف']],
  trajectory_decline:[['مصرف','دان و آب'],['محیط','دما/RH/تهویه'],['سلامت','علائم و تلفات'],['خوراک','کیفیت و تغییرات جیره']]
 };
 return patterns.map(p=>({...p,differential:(map[p.code]||[['عملکرد','بررسی روند و داده‌های مرتبط']]).map(x=>({domain:x[0],check:x[1]})),causality:'association-only'}));
}
