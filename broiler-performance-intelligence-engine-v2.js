...
function forecast(rows,m,strain){
 ...
 const pathNoise=gapReg.residualMad??0,pathTol=Math.max(.25,pathNoise*.75),rawNoise=rawReg?.residualMad??0,rawTol=Math.max(lowerIsBetter?.001:.35,rawNoise*.65);
 ...
 return{available:true,pointsUsed:vals.length,currentGapPercent:current,projectedGapPercent:projected,changePercent:delta,pathSlopePerEvaluation:pathSlope,rawSlopePerEvaluation:rawSlope,direction:pathDirection,pathDirection,rawDirection:Math.abs(rawSlope)<rawTol?'stable':rawSlope>0?'improving':'worsening',risk:pathDirection==='worsening'?'watch':pathDirection==='improving'?'improve':'stable',conditional:true,method:'reference-gap-first + intrinsic-direction + age-aware-next-evaluation',lowerIsBetter,nextEvaluationAge:nextAge,nextReferenceValue:nextReference,nextReferenceAvailable:nextReference!==null,currentPosition,projectedPosition,uncertaintyPercent:vals.length>=4?(gapReg.residualBand??null):null,evidenceLevel:vals.length>=5?'high':vals.length>=4?'medium':'limited'};
}
...
function buildForecastSummary(rows,s){
 ...
 const candidates=metricKeys.map(key=>{
   ...
   const gapEffect=t.recentGapEffect||t.gapEffect|| (Math.abs(distanceDelta)<=pathTol?'stable':current>pathTol?(distanceDelta>0?'favorable_widening':'favorable_narrowing'):current< -pathTol?(distanceDelta>0?'unfavorable_widening':'unfavorable_narrowing'):(projected>current?'toward_better':'toward_worse'));
