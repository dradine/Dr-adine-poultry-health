/* ADINE BROILER PERFORMANCE — LIFECYCLE GUARD V10
 * Presentation/lifecycle cleanup only.
 * Never hides #root and never gates Weekly/Comparison/Comprehensive rendering.
 * Removes legacy intelligence UI before it can flash underneath the V4 shell.
 */
(function(global){
'use strict';
if(global.__ADINE_BPI_LIFECYCLE_GUARD_V10__)return;
global.__ADINE_BPI_LIFECYCLE_GUARD_V10__=true;
...
function installLegacyBlock(){if(document.getElementById('adine-bpi-legacy-block-v10'))return;const s=document.createElement('style');s.id='adine-bpi-legacy-block-v10';s.textContent='#broiler-intelligence-panel,#broiler-performance-intelligence-v1-shell,.broiler-performance-intelligence-v1,.bpi-tabs-shell{display:none!important;visibility:hidden!important}';(document.head||document.documentElement).appendChild(s)}
...
function quarantineLegacy(){installLegacyBlock();document.querySelectorAll('#broiler-intelligence-panel,#broiler-performance-intelligence-v1-shell,.broiler-performance-intelligence-v1,.bpi-tabs-shell').forEach(x=>x.remove())}
...
installLegacyBlock();quarantineLegacy();
