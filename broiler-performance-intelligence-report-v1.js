...
function sparkline(model,key){
 ...
 return '<svg class="pi-spark pi-smart-gap-chart" data-pi-render-version="V11.2" data-direction-mode="'+(lowerIsBetter?'lower-is-better':'higher-is-better')+'" data-trend-direction="'+esc(t.direction||'unknown')+'" viewBox="0 0 '+w+' '+h+'" role="img" aria-label="روند '+esc(metricName(key))+' و مقایسه با '+esc(last.reference?.label||state.official?.reference?.label||'استاندارد سنی')+'">'+
   grid+
...
function trajectoryLabel(x){
 return ({recovering:'بازیابی فاصله',deteriorating:'تضعیف مسیر',converging:'همگرایی با مرجع',diverging:'دورشدن از مرجع',stable:'پایدار',volatile:'پرنوسان',insufficient:'داده ناکافی'}[x]||'—');
}
function trendDashboard(m){
 ...
 return '<div class="pi-trend-chart-grid">'+keys.map(k=>{
   ...
   const confidence=tr.pointsUsed>=5?'بالا':tr.pointsUsed>=4?'متوسط':tr.pointsUsed>=3?'محدود':'کم';
   return '<article class="pi-trend-chart-card pi-status-'+esc(status)+' '+trendClass(t.direction)+'"><div class="pi-chart-head"><b>'+esc(metricName(k))+'</b><span class="'+trendClass(t.direction)+'">'+trendIcon(t.direction)+' '+esc(t.direction==='improving'?'بهبود':t.direction==='worsening'?'افت':t.direction==='stable'?'پایدار':'داده ناکافی')+'</span></div>'+sparkline(m,k)+'<div class="pi-chart-foot"><span>'+fmt(ser.length,0)+' ارزیابی</span><span>رژیم: '+esc(regime)+'</span><span>شواهد: '+esc(confidence)+'</span></div><div class="pi-chart-reference"><b>چشم‌انداز مشروط:</b> '+esc(f?.available?(f.direction==='improving'?'فاصله در حال بهبود':f.direction==='worsening'?'فاصله در حال افت':'فاصله تقریباً ثابت'):'قابل محاسبه نیست')+' • <b>مبنای مقایسه:</b> '+esc(ser.at(-1)?.reference?.label||q?.official?.reference?.label||'استاندارد سنی معتبر')+'</div></article>';
 }).join('')+'</div>';
}
...
function outlookCard(item,kind){
 ...
 const tr=item.trajectory||{},title=kind==='risk'?'ریسک محتمل در ارزیابی بعدی':'شاخص دارای ظرفیت بهبود';
 const regime=trajectoryLabel(tr.regime);
 const reason=item.reason||'بر اساس مسیر مشاهده‌شده';
 return '<div class="pi-outlook-inner"><span>'+title+'</span><b>'+esc(item.label)+'</b><strong>'+esc(reason)+'</strong><small>رژیم روند: '+esc(regime)+' • مومنتوم: '+esc(tr.slopePerEvaluation>0?'مطلوب':tr.slopePerEvaluation<0?'نامطلوب':'خنثی')+' • شواهد: '+esc((tr.pointsUsed||0)+' ارزیابی اخیر')+'</small><p>'+esc(forecastText(item.forecast,kind))+'</p></div>';
}
...
function roadmap(m){
 ...
 return '<div class="pi-roadmap pi-roadmap-colored"><div class="pi-roadmap-step roadmap-now"><span>۱</span><div><b>اکنون</b><p>آخرین فاصله و مسیر حرکتی شاخص‌های دارای داده کافی را ثبت کن؛ تمرکز این بخش بر تغییر مسیر است، نه تکرار تحلیل چندشاخصی.</p></div></div><div class="pi-roadmap-step roadmap-next"><span>۲</span><div><b>ارزیابی بعدی</b><p>بررسی کن آیا مومنتوم حفظ شده، کند شده یا معکوس شده است؛ '+esc(imp?'ظرفیت جبران: '+imp.label:'ظرفیت جبران قابل تعیین نیست')+'.</p></div></div><div class="pi-roadmap-step roadmap-later"><span>۳</span><div><b>اگر مسیر ادامه یابد</b><p>'+esc(risk?'ریسک مسیر: '+risk.label+'. این سناریو مشروط است و پیش‌بینی قطعی نیست.':'ریسک مسیر با شواهد فعلی فعال نشده است.')+'</p></div></div></div>';
}
...
section('تحلیل چندشاخصی','جمع‌بندی چندمحوری عملکرد بر اساس همه شاخص‌های قابل اتکا',
(()=>{const mm=m.multivariateAnalysis||{},xs=m.multivariate||[],sum=mm.summary||{},axisLabel=sum.dominantAxis||'بدون محور غالب',pressure=sum.pressuredAxes?.length?sum.pressuredAxes.join(' • '):'فشار چندمحوری پررنگ دیده نشد',support=(sum.pressureEvidence||sum.supporting||[]).slice(0,6).join(' • ')||'—',protective=(sum.protective||[]).slice(0,6).join(' • ')||'—',confidence=mm.confidence||m.confidence||'پایین';const axisRows=(mm.axisSummary||[]).map(x=>{const cls=x.pressure>=1.05?'watch':x.recovery>=1.05?'good':'neutral',dir=x.pressure>=1.05?'فشار':x.recovery>=1.05?'بهبود':'متعادل';return '<div class="pi-multi-axis-row '+cls+'"><b>'+esc(x.label)+'</b><span>'+esc(dir)+'</span><small>پوشش '+esc(x.coverage)+' • فشار '+fmt(x.pressure,1)+' • روند بهبود '+fmt(x.trendRecovery??x.recovery,1)+'</small></div>'}).join('');const cards=xs.slice(0,6).map(x=>{const sev=x.severity==='high'?'high':x.severity==='watch'?'watch':'positive';return '<article class="pi-smart-insight '+sev+' pi-severity-'+esc(x.severity||'positive')+'"><div class="pi-insight-top"><span class="pi-badge">'+(x.severity==='high'?'مهم':x.severity==='watch'?'پایش':'مثبت')+'</span><b>'+esc(x.title)+'</b></div><p>'+esc(x.text)+'</p><div class="pi-multi-evidence"><b>شواهد مؤثر:</b> '+esc((x.evidence||[]).join(' • ')||'—')+'<br><b>داده لازم برای بررسی:</b> '+esc((x.checks||'—').toString())+'<br><b>محورهای قابل بررسی:</b> '+esc((x.possibleCauses||[]).join(' • ')||'—')+'</div><small>سطح شواهد: '+esc(x.confidence||confidence)+' • تفسیر ارتباطی است و علت قطعی را اثبات نمی‌کند.</small></article>'}).join('');const empty='<div class="pi-positive">با داده فعلی الگوی غالب نامطلوب یا بهبود هم‌جهت شناسایی نشد؛ این به معنی سالم بودن قطعی گله نیست.</div>';return '<div class="pi-multi-overview"><div class="pi-multi-verdict"><span>محور غالب</span><b>'+esc(axisLabel)+'</b><small>'+esc(pressure)+'</small></div><div class="pi-multi-verdict"><span>هم‌جهتی شواهد</span><b>'+fmt(sum.coherencePercent,0)+'٪</b><small>پوشش شاخص‌ها: '+esc(sum.coverage||'—')+'</small></div><div class="pi-multi-verdict"><span>شاخص‌های تحت فشار</span><b>'+esc(support)+'</b><small>شاخص‌های محافظ فعلی: '+esc(protective)+'</small></div></div><div class="pi-multi-axis-panel"><div class="pi-subtitle-row"><b>نقشه محورهای عملکرد</b><small>رشد • کارایی خوراک • بقا • یکنواختی • خروجی ترکیبی</small></div>'+axisRows+'</div><details class="pi-trend-details pi-multi-metric-trace"><summary>وضعیت ۹ شاخص در تحلیل چندشاخصی</summary><div class="pi-table-wrap"><table class="pi-trend-table"><thead><tr><th>شاخص</th><th>وضعیت فعلی</th><th>جهت روند</th><th>حرکت نسبت به مرجع</th><th>فاصله</th></tr></thead><tbody>'+(mm.metrics||[]).map(x=>'<tr class="pi-row-status-'+esc(x.status||'unavailable')+'"><td>'+esc(x.label)+'</td><td>'+esc(x.status==='critical'?'بحرانی':x.status==='watch'?'پایش':x.status==='good'?'مناسب':x.status==='excellent'?'عالی':x.status==='on_target'?'در هدف':'ناموجود')+'</td><td>'+esc(x.trend==='improving'?'بهبود':x.trend==='worsening'?'افت':x.trend==='stable'?'ثابت':'—')+'</td><td>'+esc(x.movement==='closer'?'نزدیک‌تر':x.movement==='better_farther'?'بهتر؛ فاصله بیشتر':x.movement==='worse_farther'?'بدتر؛ فاصله بیشتر':x.movement==='crossed_to_better'?'عبور به سمت بهتر':x.movement==='crossed_to_worse'?'عبور به سمت نامطلوب':x.movement==='stable'?'ثابت':'—')+'</td><td>'+esc(x.gap===null?'—':fmt(x.gap,1)+'٪')+'</td></tr>').join('')+'</tbody></table></div></details>'+(cards||empty)+'<div class="pi-trend-method">این تحلیل به‌صورت پویا از ترکیب وضعیت، فاصله از مرجع سنی، جهت روند و هم‌زمانی شاخص‌ها ساخته می‌شود و به چند سناریوی از پیش نوشته‌شده محدود نیست. استاندارد مقایسه از همان موتور کاننیکال گزارش عملکرد استفاده می‌شود و با توجه به سویه و سن ارزیابی انتخاب می‌گردد. نتیجه علت بیماری یا تشخیص مزرعه‌ای نیست و برای تصمیم نهایی باید داده‌های خوراک، آب، محیط، سلامت و مدیریت نیز بررسی شوند.</div>'})(),'multivariate')+section('روند هوشمند','مسیر عملکرد، رابطه شاخص‌ها و چشم‌انداز مشروط ارزیابی بعدی',
 '<div class="pi-trend-intro"><b>روند هوشمند چه چیز جدیدی می‌گوید؟</b><span>این بخش نتیجه تحلیل چندشاخصی را تکرار نمی‌کند؛ تمرکز آن بر «مسیر حرکت» است: مومنتوم، تداوم روند، همگرایی یا دورشدن از مرجع، نوسان و چشم‌انداز مشروط ارزیابی بعدی.</span></div>'+
 ...
 '<div class="pi-outlook"><div class="pi-outlook-risk pi-outlook-risk-v2">'+outlookCard(m.forecastSummary?.risk,'risk')+'</div><div class="pi-outlook-improve pi-outlook-improve-v2">'+outlookCard(m.forecastSummary?.improve,'improve')+'</div></div>'+
 '<div class="pi-subtitle-row pi-roadmap-title"><b>نقشه راه تصمیم</b><small>از مشاهده فعلی تا ارزیابی بعدی</small></div>'+roadmap(m)+
 '<details class="pi-trend-details"><summary>جدول شواهد و روند همه شاخص‌ها</summary><div class="pi-table-wrap"><table class="pi-trend-table"><thead><tr><th>شاخص</th><th>آخرین فاصله</th><th>شیب اخیر</th><th>حرکت نسبت به استاندارد سنی</th><th>مبنای مقایسه</th><th>رژیم روند</th><th>چشم‌انداز</th><th>تعداد نقاط</th></tr></thead><tbody>'+
 ['weight','fcr','cumulativeFcr','adg','mortality','cv','u10','u15'].map(k=>{const q=m.states?.[k]||{},t=q.trend||{},f=q.forecast||{},tr=q.trajectory||{};return '<tr class="pi-row-status-'+esc(q.official?.status||'unavailable')+'"><td>'+esc(metricName(k))+'</td><td>'+esc(q.official?.gapPercent===null?'—':fmt(q.official?.gapPercent,1)+'٪')+'</td><td>'+esc(t.available?(t.direction==='improving'?'مثبت':t.direction==='worsening'?'منفی':'تقریباً صفر'):'—')+'</td><td>'+esc(t.movement==='closer'?'نزدیک‌تر':t.movement==='better_farther'?'بهتر؛ فاصله بیشتر':t.movement==='worse_farther'?'بدتر؛ فاصله بیشتر':t.movement==='crossed_to_better'?'عبور به سمت بهتر':t.movement==='crossed_to_worse'?'عبور به سمت نامطلوب':t.movement==='stable'?'ثابت':'—')+'</td><td>'+esc(q.official?.reference?.label||'—')+'</td><td>'+esc(trajectoryLabel(tr.regime))+'</td><td>'+esc(f.available?(f.direction==='improving'?'بهبود':f.direction==='worsening'?'افت':'ثبات'):'—')+'</td><td>'+fmt(t.pointsUsed||0,0)+'</td></tr>'}).join('')+
...
global.AdineBroilerPerformanceIntelligenceReport={version:'V11.2',render};
