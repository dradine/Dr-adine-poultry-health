/* ADINE — BROILER PERFORMANCE INTELLIGENCE PRESENTATION V8.0 */
...
function metric(l,v,s,key){
 ...
 const gapText=gap===null?'مرجع در دسترس نیست':(gap>0?'+':'')+fmt(gap,1)+'٪ نسبت به مرجع';
 const dir=t.direction==='improving'?'بهبود':t.direction==='worsening'?'افت':t.direction==='stable'?'پایدار':'سابقه کافی نیست';
 const movement=t.movement==='closer'?'نزدیک‌تر به مرجع':t.movement==='farther'?'دورتر از مرجع':t.movement==='stable'?'فاصله تقریباً ثابت':'—';
...
function sparkline(model,key){
 const a=model.series?.[key]||[],f=model.states?.[key]?.forecast;
 ...
 const proj=projected===null?'':'<line class="pi-spark-proj" x1="'+lastX.toFixed(1)+'" y1="'+lastY.toFixed(1)+'" x2="'+(w-p).toFixed(1)+'" y2="'+y(projected).toFixed(1)+'"></line>';
 return '<svg class="pi-spark" viewBox="0 0 '+w+' '+h+'" role="img" aria-label="روند '+esc(metricName(key))+' نسبت به مرجع"><line class="pi-spark-zero" x1="'+p+'" y1="'+y(0)+'" x2="'+(w-p)+'" y2="'+y(0)+'"></line><path class="pi-spark-line" d="'+path+'"></path><circle class="pi-spark-last" cx="'+lastX+'" cy="'+lastY+'" r="3.5"></circle>'+proj+'</svg>';
}
...
function evidenceCard(x){
 return '<article class="pi-smart-insight '+esc(x.severity||'watch')+'"><div class="pi-insight-top"><span class="pi-badge">'+(x.severity==='high'?'مهم':x.severity==='watch'?'پایش':'مثبت')+'</span><b>'+esc(x.title)+'</b></div><p>'+esc(x.text)+'</p><small>شواهد: '+esc((x.evidence||[]).join(' • '))+'</small></article>';
}
function roadmap(m){
 const fs=m.forecastSummary||{},risk=fs.risk,imp=fs.improve;
 return '<div class="pi-roadmap"><div class="pi-roadmap-step"><span>۱</span><div><b>اکنون</b><p>تمرکز اصلی: '+esc(risk?risk.label:'پایش همه شاخص‌های اصلی')+'؛ بر اساس وضعیت فعلی و روند ثبت‌شده.</p></div></div><div class="pi-roadmap-step"><span>۲</span><div><b>ارزیابی بعدی</b><p>بررسی کن آیا روند فعلی ادامه یافته یا فاصله از مرجع تغییر جهت داده است؛ '+esc(imp?'ظرفیت بهبود: '+imp.label:'هنوز شاخص بهبود غالب مشخص نیست')+'.</p></div></div><div class="pi-roadmap-step"><span>۳</span><div><b>اگر روند ادامه یابد</b><p>'+esc(risk?'ریسک غالب: '+risk.label+'. این یک سناریوی مشروط است، نه پیش‌بینی قطعی.':'با داده فعلی ریسک غالب قابل تعیین نیست.')+'</p></div></div></div>';
}
...
section('روند هوشمند','مسیر عملکرد، رابطه شاخص‌ها و چشم‌انداز مشروط ارزیابی بعدی',
 ...
 '<div class="pi-subtitle-row"><b>روند نسبت به مرجع</b><small>خط صفر = مرجع؛ خط نقطه‌چین = چشم‌انداز مشروط</small></div>'+trendDashboard(m)+
 '<div class="pi-outlook"><div><span>ریسک محتمل در ارزیابی بعدی</span><b>'+esc(m.forecastSummary?.risk?.label||'قابل تعیین نیست')+'</b><small>'+esc(forecastText(m.forecastSummary?.risk?.forecast))+'</small></div><div><span>شاخص دارای ظرفیت بهبود</span><b>'+esc(m.forecastSummary?.improve?.label||'قابل تعیین نیست')+'</b><small>'+esc(forecastText(m.forecastSummary?.improve?.forecast))+'</small></div></div>'+
 '<div class="pi-subtitle-row"><b>نقشه راه تصمیم</b><small>از مشاهده فعلی تا ارزیابی بعدی</small></div>'+roadmap(m)+
 '<details class="pi-trend-details"><summary>جدول شواهد و روند همه شاخص‌ها</summary><div class="pi-table-wrap"><table class="pi-trend-table"><thead><tr><th>شاخص</th><th>آخرین فاصله</th><th>شیب اخیر</th><th>حرکت نسبت به مرجع</th><th>چشم‌انداز</th><th>تعداد نقاط</th></tr></thead><tbody>'+
 ['weight','fcr','cumulativeFcr','adg','mortality','cv','u10','u15'].map(k=>{const q=m.states?.[k]||{},t=q.trend||{},f=q.forecast||{};return '<tr><td>'+esc(metricName(k))+'</td><td>'+esc(q.official?.gapPercent===null?'—':fmt(q.official?.gapPercent,1)+'٪')+'</td><td>'+esc(t.available?(t.direction==='improving'?'مثبت':t.direction==='worsening'?'منفی':'تقریباً صفر'):'—')+'</td><td>'+esc(t.movement==='closer'?'نزدیک‌تر':t.movement==='farther'?'دورتر':t.movement==='stable'?'ثابت':'—')+'</td><td>'+esc(f.available?(f.direction==='improving'?'بهبود':f.direction==='worsening'?'افت':'ثبات'):'—')+'</td><td>'+fmt(t.pointsUsed||0,0)+'</td></tr>'}).join('')+
 '</tbody></table></div></details>'+
 '<div class="pi-trend-method">روش تحلیل: روندهای اخیر نسبت به مرجع مشترک، الگوهای چندشاخصی و پیش‌بینی مقاوم به‌صورت مشروط بررسی می‌شوند. با داده کم یا نوسان زیاد، سطح اطمینان کاهش می‌یابد و نتیجه قطعی اعلام نمی‌شود.</div>')+section('هشدار تطبیقی','تشخیص تغییر غیرعادی نسبت به سابقه خود گله، همراه با شواهد و اولویت بررسی',
 (()=>{
  const warnings=[];
  ...
  (m.trendInsights||[]).filter(x=>x.severity==='high').forEach(x=>warnings.push({key:x.type,name:x.title,kind:'pattern',x}));
...
global.AdineBroilerPerformanceIntelligenceReport={version:'V8.0',render};
