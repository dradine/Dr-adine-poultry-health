...
(()=>{
 ...
 return '<div class="pi-smart-summary">'+xs.slice(0,8).map((x,i)=>{
  ...
  return '<article class="pi-smart-insight '+sev+'"><div class="pi-insight-top"><span class="pi-badge">'+(x.severity==='high'?'مهم':x.severity==='watch'?'پایش':'مثبت')+'</span><b>'+esc(x.title)+'</b></div><p>'+esc(x.text)+'</p><div class="pi-multi-evidence"><b>شواهد درگیر:</b> '+esc((x.evidence||[]).join(' • '))+'<br><b>علل قابل بررسی:</b> '+esc(causes)+'<br><b>داده تکمیلی لازم:</b> '+esc(checks)+'</div><small>سطح شواهد: '+esc(x.confidence||m.confidence)+' • این رابطه، تفسیری/ارتباطی است و علت قطعی را اثبات نمی‌کند.</small></article>';
...
global.AdineBroilerPerformanceIntelligenceReport={version:'V9.0',render};
