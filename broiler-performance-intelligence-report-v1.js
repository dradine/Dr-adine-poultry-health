...
(()=>{const mm=m.multivariateAnalysis||{},xs=m.multivariate||[],sum=mm.summary||{},axisLabel=sum.dominantAxis||'بدون محور غالب',pressure=sum.pressuredAxes?.length?sum.pressuredAxes.join(' • '):'فشار چندمحوری پررنگ دیده نشد',support=(sum.supporting||[]).slice(0,6).join(' • ')||'—',protective=(sum.protective||[]).slice(0,6).join(' • ')||'—',confidence=mm.confidence||m.confidence||'پایین';const axisRows=(mm.axisSummary||[]).map(x=>{const cls=x.pressure>=1.05?'watch':x.recovery>=1.05?'good':'neutral',dir=x.pressure>=1.05?'فشار':x.recovery>=1.05?'بهبود':'متعادل';return '<div class="pi-multi-axis-row '+cls+'"><b>'+esc(x.label)+'</b><span>'+esc(dir)+'</span><small>پوشش '+esc(x.coverage)+' • فشار '+fmt(x.pressure,1)+' • بهبود '+fmt(x.recovery,1)+'</small></div>'}).join('');const cards=xs.slice(0,6).map(x=>{const sev=x.severity==='high'?'high':x.severity==='watch'?'watch':'positive';return '<article class="pi-smart-insight '+sev+' pi-severity-'+esc(x.severity||'positive')+'"><div class="pi-insight-top"><span class="pi-badge">'+(x.severity==='high'?'مهم':x.severity==='watch'?'پایش':'مثبت')+'</span><b>'+esc(x.title)+'</b></div><p>'+esc(x.text)+'</p><div class="pi-multi-evidence"><b>شواهد مؤثر:</b> '+esc((x.evidence||[]).join(' • ')||'—')+'<br><b>داده لازم برای بررسی:</b> '+esc((x.checks||'—').toString())+'<br><b>محورهای قابل بررسی:</b> '+esc((x.possibleCauses||[]).join(' • ')||'—')+'</div><small>سطح شواهد: '+esc(x.confidence||confidence)+' • تفسیر ارتباطی است و علت قطعی را اثبات نمی‌کند.</small></article>'}).join('');const empty='<div class="pi-positive">با داده فعلی الگوی غالب نامطلوب یا بهبود هم‌جهت شناسایی نشد؛ این به معنی سالم بودن قطعی گله نیست.</div>';return '<div class="pi-multi-overview"><div class="pi-multi-verdict"><span>محور غالب</span><b>'+esc(axisLabel)+'</b><small>'+esc(pressure)+'</small></div><div class="pi-multi-verdict"><span>هم‌جهتی شواهد</span><b>'+fmt(sum.coherencePercent,0)+'٪</b><small>پوشش شاخص‌ها: '+esc(sum.coverage||'—')+'</small></div><div class="pi-multi-verdict"><span>شواهد مؤید فشار</span><b>'+esc(support)+'</b><small>شاخص‌های محافظ: '+esc(protective)+'</small></div></div><div class="pi-multi-axis-panel"><div class="pi-subtitle-row"><b>نقشه محورهای عملکرد</b><small>رشد • کارایی خوراک • بقا • یکنواختی • خروجی ترکیبی</small></div>'+axisRows+'</div>'+(cards||empty)+'<div class="pi-trend-method">این تحلیل به‌صورت پویا از ترکیب وضعیت، فاصله از مرجع سنی، جهت روند و هم‌زمانی شاخص‌ها ساخته می‌شود و به چند سناریوی از پیش نوشته‌شده محدود نیست. استاندارد مقایسه از همان موتور کاننیکال گزارش عملکرد استفاده می‌شود و با توجه به سویه و سن ارزیابی انتخاب می‌گردد. نتیجه علت بیماری یا تشخیص مزرعه‌ای نیست و برای تصمیم نهایی باید داده‌های خوراک، آب، محیط، سلامت و مدیریت نیز بررسی شوند.</div>})(),'multivariate')+section('روند هوشمند','مسیر عملکرد، رابطه شاخص‌ها و چشم‌انداز مشروط ارزیابی بعدی',
