...
function metric(l,v,s,key){
 ...
 const movement=t.movement==='closer'?'نزدیک‌تر به همان استاندارد سنی':t.movement==='farther'?'دورتر از همان استاندارد سنی':t.movement==='stable'?'فاصله از استاندارد سنی تقریباً ثابت':'—';
...
function sparkline(model,key){
 ...
 return '<svg class="pi-spark" viewBox="0 0 '+w+' '+h+'" role="img" aria-label="روند '+esc(metricName(key))+' نسبت به '+esc(ref.label||'استاندارد سنی')+'"><line class="pi-spark-zero" x1="'+p+'" y1="'+y(0)+'" x2="'+(w-p)+'" y2="'+y(0)+'"></line><path class="pi-spark-line" d="'+path+'"></path><circle class="pi-spark-last" cx="'+lastX+'" cy="'+lastY+'" r="3.5"></circle>'+proj+'</svg>';
}
...
(()=>{
 ...
 '<div class="pi-subtitle-row pi-subtitle-reference"><b>روند نسبت به استاندارد سنی</b><small>خط صفر = استاندارد همان سن؛ خط نقطه‌چین = چشم‌انداز مشروط</small></div>'+trendDashboard(m)+
