...
(function(global){
  ...
  function managementWeeklyFcr(flock,rows,index){const current=standardFor(flock,rows[index]);if(!current||current.fcr===null)return null;if(index===0)return n(current.fcr);const previous=standardFor(flock,rows[index-1]);const cw=n(current.weight),pw=n(previous?.weight),cc=n(current.fcr),pc=n(previous?.fcr);if([cw,pw,cc,pc].some(x=>x===null)||cw<=pw)return null;return (cc*cw-pc*pw)/(cw-pw)}
  function managementWeightGain(flock,rows,index){const current=standardFor(flock,rows[index]);if(!current||current.weight===null)return null;if(index===0){const initial=n(flock.initial_average_weight_g);return initial===null?null:n(current.weight)-initial}const previous=standardFor(flock,rows[index-1]);return previous?.weight===null?null:n(current.weight)-n(previous.weight)}
