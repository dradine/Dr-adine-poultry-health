/* ADINE — Stable latest-week display + report-view stability V4
   Presentation only. Uses the same latest/common comparison week already produced by the comparison report.
*/
"use strict";
(function(global){
  ...
  async function comparison(){try{
    /* The comparison engine already determines the latest common week. Reuse that exact value. */
    ...
    const common=Math.min(...weeks);
    set(`<span>ارزیابی آخرین هفته</span><strong>هفته ${fmt(common)}</strong>`,true);
  }catch(e){set('<span>ارزیابی آخرین هفته</span><strong>—</strong>',true)}}
  function patchComparisonWeek(){if(patching||activeTab()!=='compare-empty')return;const select=$('#fc2-week');if(!select||select.dataset.autoWeek==='1')return;const selected=select.selectedOptions?.[0];if(!selected)return;const text=selected.textContent.trim(),match=text.match(/هفته\s+([۰-۹0-9]+)/),week=match?match[1]:text;patching=true;const span=document.createElement('span');span.id='fc2-week';span.className='fc-week fc-week-auto';span.setAttribute('aria-label','هفته ارزیابی خودکار');span.textContent=`هفته ارزیابی: ${week}`;select.replaceWith(span);patching=false}
  function sync(){const t=activeTab();if(t==='overall')overall();else if(t==='compare-empty')comparison();else set('',false)}
  document.addEventListener('click',e=>{const tab=e.target?.closest?.('.report-tab');if(tab&&(tab.dataset.tab==='overall'||tab.dataset.tab==='compare-empty'))prepare(tab.dataset.tab);if(tab)setTimeout(sync,0);if(e.target?.closest?.('#fc2-run'))[100,500,1000].forEach(ms=>setTimeout(comparison,ms));if(e.target?.closest?.('#fc2-clear'))setTimeout(comparison,50)},true);
  document.addEventListener('change',e=>{if(e.target?.matches?.('[id^="fc2-flock-"]')||e.target?.matches?.('#fc2-week'))setTimeout(comparison,0)});
  const rootObserver=new MutationObserver(()=>{if(!patching){patchComparisonWeek();if(activeTab()==='compare-empty')comparison()}});
