...
(function(global){'use strict';
  ...
  function weeklyFCR(input){
    ...
    const prev={live_birds:num(p.openBirds??p.open_birds??flock.initial_bird_count??flock.initialBirdCount),average_weight_g:num(p.openWeight??p.open_weight_g??flock.initial_average_weight_g??flock.initialAverageWeightG),age_days:num(p.openAgeDays??p.open_age_days)??0};
    const curr={live_birds:num(p.closeBirds??p.close_birds),average_weight_g:num(p.closeWeight??p.close_weight_g),feed_total_kg:num(p.feedKg??p.feed_total_kg??p.feedTotalKg),age_days:num(p.closeAgeDays??p.close_age_days)??1};
    ...
    const out=canonicalRows([prev,curr],flock);return out.length?out[out.length-1].weeklyFcr:null;
  }
