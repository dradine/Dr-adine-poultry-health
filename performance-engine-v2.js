...
(function(global){'use strict';
  ...
  function weeklyFCR(input){const p=input||{},flock=flockFor();const prev={live_birds:num(p.openBirds??p.open_birds??flock.initial_bird_count??flock.initialBirdCount),average_weight_g:num(p.openWeight??p.open_weight_g??flock.initial_average_weight_g??flock.initialAverageWeightG),age_days:num(p.openAgeDays??p.open_age_days)??0};const curr={live_birds:num(p.closeBirds??p.close_birds),average_weight_g:num(p.closeWeight??p.close_weight_g),feed_total_kg:num(p.feedKg??p.feed_total_kg??p.feedTotalKg),age_days:num(p.closeAgeDays??p.close_age_days)??1};if(prev.live_birds===null||prev.live_birds<=0||prev.average_weight_g===null||prev.average_weight_g<=0||curr.live_birds===null||curr.live_birds<=0||curr.average_weight_g===null||curr.average_weight_g<=0||curr.feed_total_kg===null||curr.feed_total_kg<=0)return null;const out=canonicalRows([prev,curr],flock);return out.length?out[out.length-1].weeklyFcr:null;}
  ...
  function loadBridge(){if(typeof document==='undefined'||document.querySelector('script[data-adine-broiler-fcr-weekly-ui="1"]'))return;const s=document.createElement('script');s.src='broiler-fcr-weekly-ui-v1.js?v=1.8.0';s.async=false;s.dataset.adineBroilerFcrWeeklyUi='1';(document.head||document.documentElement).appendChild(s);}
  if(typeof document!=='undefined'){if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',loadBridge,{once:true});else loadBridge();}
