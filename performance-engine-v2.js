...
(function(global){'use strict';
  ...
  function weeklyFCR(input){
    const p=input||{};
    const prev={live_birds:num(p.openBirds??p.open_birds),average_weight_g:num(p.openWeight??p.open_weight_g),age_days:num(p.openAgeDays??p.open_age_days)??0};
    ...
    const out=canonicalRows([prev,curr],flockFor());
  ...
  global.AdinePerformance={version:'CANONICAL-ADAPTER-4',typeOf,rows,canonicalRows,broilerWeeklyFCR:weeklyFCR,broilerCumulativeFCR:(records,flock)=>cumulativeFCR(records,null,flock),latestWeekly:(records,flock)=>{const out=canonicalRows(records,flock);return out.length?out[out.length-1].weeklyFcr:null},latestCumulative:(records,flock)=>{const out=canonicalRows(records,flock);return out.length?out[out.length-1].cumulativeFcr:null},quality:x=>({ok:true,issues:[]})};
