/* ADINE REPORTS — FINAL STANDARD PRESENTATION GUARD V2
...
(function(){
  ...
  function apply(){const s=get(),r=root();if(!s||!r)return;r.querySelectorAll('.metric').forEach(c=>{const l=c.querySelector('.metric-label')?.textContent.trim();if(l==='میانگین وزن')ref(c,`استاندارد رسمی: ${fa(s.w,1)} گرم`);else if(l==='افزایش وزن هفتگی'){const old=c.querySelector('.metric-ref')?.textContent||'';const m=old.match(/هدف مدیریتی:\s*([۰-۹0-9٬,٫.]+)/);ref(c,m?`هدف مدیریتی: ${m[1]} گرم — بر پایه استاندارد رسمی`:`هدف مدیریتی: ${fa(s.gain,1)} گرم — بر پایه استاندارد رسمی`)}else if(l==='CV')ref(c,'هدف مدیریتی: ≤ ۱۰٪ — کمتر بهتر است');else if(l==='یکنواختی ±10')ref(c,'هدف مدیریتی: ≥ ۸۰٪ — بیشتر بهتر است');else if(l==='یکنواختی ±15')ref(c,'هدف مدیریتی: ≥ ۹۰٪ — بیشتر بهتر است');else if(l==='FCR هفتگی')ref(c,`استاندارد رسمی هفتگی: ${fa(s.wf,3)} — هدف مدیریتی: ${fa(s.wf,3)}`);else if(l==='FCR تجمعی')ref(c,`استاندارد رسمی تجمعی: ${fa(s.cum,3)}`);});}
  function boot(){let last='';const tick=()=>{const r=root();const sig=(r?.innerText||'').slice(0,500);if(sig!==last){last=sig;apply()}};tick();new MutationObserver(tick).observe(document.body,{childList:true,subtree:true,characterData:true});setInterval(apply,1200)}
