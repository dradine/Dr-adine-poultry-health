/* ADINE REPORTS — STRAIN-AWARE STANDARD LABELS V1
   UI-only layer. Does not alter calculations or canonical weekly records.
*/
"use strict";
(function(){
  const $=s=>document.querySelector(s);
  const fa=v=>Number(v).toLocaleString('fa-IR',{minimumFractionDigits:3,maximumFractionDigits:3});
  ...
  function setRef(card,text){let r=card.querySelector('.metric-ref');if(!r){r=document.createElement('div');r.className='metric-ref';card.appendChild(r)}r.textContent=text}
  function refresh(){
    ...
    root.querySelectorAll('.metric').forEach(card=>{
      ...
      if(label==='میانگین وزن' && s?.weight!=null) setRef(card,`استاندارد رسمی ${fa(s.weight)} گرم`);
      else if(label==='FCR تجمعی' && s?.fcr!=null) setRef(card,`استاندارد رسمی تجمعی ${fa(s.fcr)}`);
      else if(label==='FCR هفتگی' && s?.fcr!=null){const old=card.querySelector('.metric-ref')?.textContent||'';const mg=old.match(/هدف مدیریتی:\s*([^ ]+)/)?.[1];setRef(card,`استاندارد رسمی: ${fa(s.fcr)} — هدف مدیریتی: ${mg||fa(s.fcr)}`)}
