/* ADINE REPORTS — BROILER PERFORMANCE INTELLIGENCE INTEGRATION V2
   Read-only UI integration. Does not calculate or write canonical production metrics.
   Mounts after the report renderer so the intelligence panel is not lost when #root is re-rendered.
*/
"use strict";
(function(global){
  ...
  async function run(){
    const hostExisting=$("broiler-intelligence-panel");
    if(hostExisting)return;
    if(!global.AdinePerformanceIntelligence||!global.AdineReportRouter||!global.AdineBroilerReportEngine)return;
    const root=$("root"),sel=$("week");if(!root||!sel)return;
    const active=document.querySelector(".report-tab.active");
    if(active&&active.dataset.tab&&active.dataset.tab!=="weekly")return;
    const flockId=global.AdineReportRouter.currentFlockId();if(!flockId)return;
    let flock,rawRows;
    try{[flock,rawRows]=await Promise.all([global.AdineReportRouter.getFlock(flockId),global.AdineReportRouter.getWeeklyRecords(flockId)])}
    catch(e){console.warn("Broiler intelligence integration data load failed",e);return}
    if(global.AdineReportRouter.productionType(flock.production_type)!=="broiler")return;
    const model=global.AdineBroilerReportEngine.build(flock,rawRows||[]),i=Number(sel.value||0),r=model.rows[i];if(!r)return;
    ...
    const results=await Promise.all(metrics.map(async m=>{
      const current=valueFor(r,m),target=targetFor(r,m);
      if(current===null||target===null)return{ok:false,code:"insufficient_data",metric:m};
      const fs=futureStandard(model.rows,i,m);
    ...
    const host=document.createElement("section");host.id="broiler-intelligence-panel";host.className="section broiler-intelligence";
  ...
  let timer=null;
  function schedule(){clearTimeout(timer);timer=setTimeout(()=>run().catch(e=>console.warn("Broiler intelligence render failed",e)),120)}
  document.addEventListener("change",e=>{if(e.target?.id==="week")schedule()});
  document.addEventListener("click",e=>{if(e.target?.closest?.(".report-tab"))setTimeout(schedule,100)});
  function start(){schedule();const root=$("root");if(root){const observer=new MutationObserver(()=>{if(!$("broiler-intelligence-panel"))schedule()});observer.observe(root,{childList:true});}}
