...
(function(global){
  ...
  function card(a,m){
    ...
    if(!a?.ok){
      const mg=management?`<div class="pi-target management">هدف مدیریتی: ${fmt(management.value,m.includes("fcr")?3:1)}${unit(m)}${management.sourceName?` — ${esc(management.sourceName)}`:''}</div>`:'';
