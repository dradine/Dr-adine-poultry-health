...
(function(global){
  ...
  async function getCanonicalFcrTargets(flockId){if(!flockId||!global.supabaseClient?.rpc)return{byRecord:new Map(),available:false,error:null};try{const {data,error}=await global.supabaseClient.rpc('get_flock_fcr_analysis_v4',{p_flock_id:flockId});if(error)throw error;const byRecord=new Map((data||[]).map(r=>[String(r.record_id),r]));return{byRecord,available:true,error:null}}catch(error){console.warn('Canonical broiler FCR target reader unavailable:',error);return{byRecord:new Map(),available:false,error}}}
