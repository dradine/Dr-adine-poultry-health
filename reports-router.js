...
(function(global){
  ...
  async function getCanonicalFcrTargets(flockId){
    if(!flockId||!global.supabaseClient?.rpc)return{byRecord:new Map(),available:false,error:null};
    try{
      const {data,error}=await global.supabaseClient.rpc('get_flock_fcr_analysis_v4',{p_flock_id:flockId});
      if(error)throw error;
      const byRecord=new Map((data||[]).map(r=>[String(r.record_id),r]));
      return{byRecord,available:true,error:null};
    }catch(error){
      console.warn('Canonical broiler FCR target reader unavailable:',error);
      return{byRecord:new Map(),available:false,error};
    }
  }

  async function getWeeklyRecords(flockId){
    const {data,error}=await global.supabaseClient.from('weekly_records').select('id,flock_id,record_date,evaluation_date,age_days,week_number,bird_count,live_birds,mortality_count,mortality,livability,sample_count,average_weight,average_weight_g,sd,sd_weight_g,cv,cv_percent,uniformity_10,uniformity_10_percent,uniformity_15,uniformity_15_percent,min_weight,min_weight_g,max_weight,max_weight_g,feed,feed_total_kg,feed_per_bird_g,water,water_total_liter,water_per_bird_ml,fcr,cumulative_fcr,standard_weight,standard_difference,standard_difference_percent,water_feed_ratio,production_metrics,notes,created_at,updated_at,production_day,production_week,flock_phase,timeline_version').eq('flock_id',flockId).order('week_number',{ascending:true});
    if(error)throw error;
    const rows=data||[];
    const targetResult=await getCanonicalFcrTargets(flockId);
    if(!targetResult.available)return rows;
    return rows.map(r=>{
      const t=targetResult.byRecord.get(String(r.id));
      if(!t)return r;
      return Object.freeze({...r,
        officialWeeklyFcr:t.official_weekly_fcr??null,
        officialCumulativeFcr:t.official_cumulative_fcr??null,
        managementWeeklyFcr:t.management_weekly_fcr??null,
        managementCumulativeFcr:t.management_cumulative_fcr??null,
        officialFcrSource:t.official_source??null,
        officialFcrYear:t.official_year??null,
        managementFcrSource:t.management_cohort??null,
        managementFcrSourceVersion:t.calculation_version??null,
        targetAuthority:'canonical-weekly-report',
        targetRecordId:t.record_id??r.id
      });
    });
  }
