-- Broiler performance-intelligence only.
-- Preserve canonical production/FCR calculations; normalize strain spacing in management fallback lookup.
-- This fixes labels such as "Cobb 500" vs "Cobb500" without changing actual flock metrics.
...
begin
  ...
  if lower(trim(coalesce(p_production_type,''))) in ('broiler','گوشتی','meat') and m in ('fcr_weekly','fcr_cumulative') then
    ...
    if target is null then
      ...
      where x.active and lower(trim(coalesce(x.production_type,'')))=lower(trim(coalesce(p_production_type,''))) and lower(trim(x.metric_code))=m and x.age_days=p_age_days
        and (p_genetics is null or x.genetics is null or lower(trim(x.genetics))=lower(trim(p_genetics)))
        and (p_strain is null or x.strain is null or regexp_replace(lower(trim(x.strain)),'[[:space:]_.-]+','','g')=regexp_replace(lower(trim(p_strain)),'[[:space:]_.-]+','','g'))
