-- Canonical broiler FCR V4
-- Align stored weekly/cumulative FCR with breeder-catalog semantics:
-- FCR does not separately account for mortality. Period/cumulative gain is
-- calculated on the current live population multiplied by per-bird weight gain.
...
begin
  ...
  if v_cum_feed>0 and new.live_birds is not null and new.live_birds>0 and v_current_weight is not null and f.initial_average_weight_g is not null and v_current_weight>f.initial_average_weight_g then
    v_cumulative_fcr := round((v_cum_feed/((new.live_birds*(v_current_weight-f.initial_average_weight_g))/1000.0))::numeric,4);
  end if;
  ...
  metrics := coalesce(new.production_metrics,'{}'::jsonb)
    || jsonb_build_object(
      'calculation_version','BROILER-FCR-CANONICAL-V4',
      'calculation_scope','week_scoped',
      'selected_week',v_week,
      'weekly_fcr',v_weekly_fcr,
      'cumulative_fcr',v_cumulative_fcr,
      'weekly_fcr_semantics','period_feed_kg / (current_live_birds × period_per_bird_weight_gain_g / 1000)',
      'cumulative_fcr_semantics','cumulative_feed_kg / (current_live_birds × total_per_bird_weight_gain_from_placement_g / 1000)',
      'mortality_in_comparable_fcr',false,
      'official_fcr_comparison_basis','Breeder performance-objective FCR; mortality not separately accounted for'
    );
...
where exists (
  ...
  where f.id=w.flock_id
    and lower(coalesce(f.production_type,'')) in ('broiler','گوشتی','meat')
);