...
const S=sandbox.AdineBroilerPerformanceIntelligenceSourceV93;
assert(S&&S.version==='BROILER-PI-SOURCE-V9.4');
assert.strictEqual(S.sourceProtocol,'PI-SOURCE-V93');
...
assert(C&&C.version==='BROILER-PI-SOURCE-V9.4','canonical source must expose the isolated V93 runtime');
assert.strictEqual(C.sourceProtocol,'PI-SOURCE-V93');
assert.strictEqual(C.enrich({strain:'Ross 308 AP'},rows)[0].canonicalTargets.weight,2360,'canonical source must use the canonical target resolver');

console.log('BROILER PERFORMANCE INTELLIGENCE SOURCE V9.4 / ISOLATED V93 TESTS: PASS');
