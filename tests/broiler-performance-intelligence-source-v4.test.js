...
assert(S&&S.version==='BROILER-PI-SOURCE-V8');
...
assert.strictEqual(out.canonicalTargets.fcr,1.32,'Explicit canonical weekly target remains authoritative when metric layer is unavailable in the isolated test sandbox');
assert.strictEqual(out.targetAuthority,'canonical-weekly-report');
assert.strictEqual(out.targetResolver,'broiler-canonical-metric-resolver-v1');
console.log('BROILER PERFORMANCE INTELLIGENCE SOURCE V8 TESTS: PASS');
