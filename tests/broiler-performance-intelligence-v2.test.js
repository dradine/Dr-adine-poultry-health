...
assert(E&&E.version==='BROILER-PI-V3.1');
...
// 1 raw FCR worsens but target-relative FCR improves.
...
m=E.build({},[row({week:4,age:28,weight:1000,standardWeight:1000,managementWeight:1000}),row({week:5,age:35,weight:1020,standardWeight:1100,managementWeight:1100})]);assert(m.states.weight.rawChange>0);assert.strictEqual(m.states.weight.official.trend.direction,'worsening');
...
m=E.build({},[row({week:4,age:28,weight:1000,standardWeight:1000,managementWeight:900}),row({week:5,age:35,weight:1020,standardWeight:1000,managementWeight:1100})]);assert.strictEqual(m.states.weight.official.trend.direction,'improving');assert.strictEqual(m.states.weight.management.trend.direction,'worsening');
...
// 9 duplicate weeks/ages and out-of-order input are reported before sorting.
...
// 10 negative values are invalid data.
m=E.build({},[row({week:1,age:7,weight:-1})]);assert(m.validation.some(x=>x.type==='negative_value'));
// 11 cumulative FCR uses its own current/previous targets.
...
// 12 raw change is descriptive only and never determines trend.
assert.strictEqual(m.rawTrendIsDescriptiveOnly,true);assert(Object.prototype.hasOwnProperty.call(m.states.fcr,'rawChange'));
// 13 exact target is on-target and presentation status must not be a warning.
m=E.build({},[row({week:1,age:7,weight:1000,standardWeight:1000,managementWeight:1000})]);assert.strictEqual(m.states.weight.official.relativeStatus,'on_target');assert.strictEqual(m.states.weight.official.status,'good');assert.strictEqual(m.states.weight.official.deviation,0);
// 14 current/previous target fields are paired to the same row.
...
// 15 duplicate evaluation dates are a data-quality issue.
...
// 16 management target is unavailable when canonical management data is absent; it is never copied from official.
...
// 17 non-canonical target provenance is rejected by the intelligence layer.
m=E.build({},[row({week:1,age:7,standardWeight:1200,managementWeight:1200,targetAuthority:'external-source'})]);assert.strictEqual(m.states.weight.official.target,null);assert.strictEqual(m.states.weight.management.target,null);assert.strictEqual(m.states.weight.official.status,'unavailable');
// 18 canonical provenance is accepted and exposed.
m=E.build({},[row({week:1,age:7,standardWeight:1200,managementWeight:1100,targetAuthority:'canonical-weekly-report'})]);assert.strictEqual(m.targetAuthority,'canonical-weekly-report');assert.strictEqual(m.states.weight.official.target,1200);assert.strictEqual(m.states.weight.management.target,1100);
// 19 raw deviation is signed actual-vs-target, while gap remains performance-direction-aware.
m=E.build({},[row({week:1,age:7,weight:1100,standardWeight:1000})]);assert.strictEqual(m.states.weight.official.deviation,10);assert.strictEqual(m.states.weight.official.gap,0.1);
// 20 official and management interpretations are both generated independently when both targets exist.
m=E.build({},[row({week:1,age:7,weight:900,standardWeight:1000,managementWeight:800})]);assert(m.insights.length>=1);assert(Array.isArray(m.managementInsights));
console.log('BROILER PERFORMANCE INTELLIGENCE V3.1 TESTS: PASS');
