...
class MockRecognition {
  constructor() { MockRecognition.instance = this; this.startCalls = 0; }
...
const context = {
  ...
  console, Event: function () {}, CSS: { escape: x => x }, setTimeout, clearTimeout
};
context.window.window = context.window; context.window.document = document;
context.window.MutationObserver = MutationObserver; context.window.CSS = context.CSS;
...
assert(api && api.version === '1.1.0');
assert.strictEqual(api.supported, true);

const cases = [
  ['1234', '۱۲۳۴'], ['٣٤٥', '۳۴۵'], ['سه درصد', '۳ درصد'],
  ...
  ['پی سی آر و ال ای زا', 'PCR و ELISA'], ['اف سی آر و سی وی', 'FCR و CV'],
  ['کیو پی سی آر و آر تی پی سی آر', 'qPCR و RT-PCR'], ['اچ وی تی و آی بی دی', 'HVT و IBD'],
  ['پودو درماتیت و آنتی بیوگرام', 'پودودرماتیت و آنتی‌بیوگرام'],
...
const field = makeElement('textarea'); field.parentElement = parent;
api.attach(field);
assert.strictEqual(field.dataset.adineVoiceAttached, '1');
assert(lastButton && typeof lastButton.on_click === 'function');
lastButton.on_click();
let r = MockRecognition.instance;
assert.strictEqual(r.lang, 'fa-IR');
...
r.onresult({ resultIndex: 0, results: [
  { isFinal: false, 0: { transcript: 'سه هزار قطعه' } }
]});
assert.strictEqual(field.value, '۳۰۰۰ قطعه');
r.onresult({ resultIndex: 0, results: [
  { isFinal: true, 0: { transcript: ' سه هزار قطعه' } },
  { isFinal: false, 0: { transcript: ' درگیری تنفسی' } }
]});
assert(field.value.includes('۳۰۰۰ قطعه'));
assert(field.value.includes('درگیری تنفسی'));
const once = field.value;
r.onresult({ resultIndex: 1, results: [
  { isFinal: true, 0: { transcript: ' سه هزار قطعه' } },
  { isFinal: true, 0: { transcript: ' درگیری تنفسی' } }
]});
assert.strictEqual(field.value, once.replace('درگیری تنفسی','درگیری تنفسی'));
lastButton.on_click();
...
for (const id of ['farmCode','flockCode','otp','password','phone','email','username']) {
  const blocked = makeElement('input'); blocked.id = id; blocked.parentElement = parent; api.attach(blocked);
  assert.notStrictEqual(blocked.dataset.adineVoiceAttached, '1', `blocked field ${id}`);
}

const readonly = makeElement('textarea'); readonly.readOnly = true; readonly.parentElement = parent; api.attach(readonly);
assert.notStrictEqual(readonly.dataset.adineVoiceAttached, '1');
const disabled = makeElement('textarea'); disabled.disabled = true; disabled.parentElement = parent; api.attach(disabled);
assert.notStrictEqual(disabled.dataset.adineVoiceAttached, '1');

console.log(`voice-input hardening tests passed: ${cases.length} normalization cases + lifecycle + eligibility + duplicate-final protection`);
