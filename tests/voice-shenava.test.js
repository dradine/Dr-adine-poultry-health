// Final CI trigger: run the complete local Shenava regression suite on the current main implementation.
const fs = require('fs');
const vm = require('vm');
...
const cases = [
  ['۱۲۳۴', '۱۲۳۴'], ['سه هزار و پانصد قطعه', '۳۵۰۰ قطعه'], ['دو میلیون و سیصد هزار', '۲۳۰۰۰۰۰'],
  ['یک و نیم کیلو', '۱٫۵ کیلو'], ['سه ممیز پنج درصد', '۳٫۵ درصد'],
  ['مایکو پلازما و کیسه های هوایی', 'مایکوپلاسما و کیسه‌های هوایی'], ['اف سی آر و سی وی', 'FCR و CV'],
  ['پی سی آر و ال ای زا', 'PCR و ELISA'], ['کیو پی سی آر و آر تی پی سی آر', 'qPCR و RT-PCR'],
  ...
  ['تولید روز مرغ و تولید مرغ روز', 'تولید روزمرغ و تولید روزمرغ'], ['تخم کف بستر، تخم شکسته و استحکام پوسته', 'تخم کف بستر، تخم شکسته و استحکام پوسته'],
  ['باروری، نطفه داری و درصد هچ', 'باروری، نطفه‌داری و درصد هچ'], ['گله والد، نسبت خروس به ماده و اسپایکینگ', 'گله والد، نسبت خروس به ماده و اسپایکینگ'],
...
for (const [input, expected] of cases) assert.strictEqual(api.normalize(input), expected, `normalize failed: ${input}`);
