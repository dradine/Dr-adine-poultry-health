...
(function () {
  ...
  function digits(v){return String(v==null?'':v).replace(/[0-9]/g,d=>'۰۱۲۳۴۵۶۷۸۹'[+d]).replace(/[٠-٩]/g,d=>'۰۱۲۳۴۵۶۷۸۹'[d.charCodeAt(0)-0x0660]);}
  function parseSpokenNumber(words){let total=0,current=0,used=false;for(const w of words){if(Object.prototype.hasOwnProperty.call(WORDS,w)){current+=WORDS[w];used=true;continue;}if(w==='و')continue;if(Object.prototype.hasOwnProperty.call(SCALES,w)){total+=(current||1)*SCALES[w];current=0;used=true;continue;}return null;}return used?total+current:null;}

  function normalizeTerms(value){
    let text=String(value==null?'':value).replace(/[يى]/g,'ی').replace(/ك/g,'ک').replace(/[ـ]/g,'').replace(/[ \t]+/g,' ').replace(/\s+([،؛؟.!])/g,'$1').trim();
    const protectedPairs=[[/کیو\s*پی\s*سی\s*آر/gi,'__QPCR__'],[/آر\s*تی\s*پی\s*سی\s*آر/gi,'__RTPCR__'],[/پی\s*سی\s*آر/gi,'__PCR__'],[/ال\s*ای\s*زا/gi,'__ELISA__'],[/اف\s*سی\s*آر/gi,'__FCR__'],[/سی\s*وی/gi,'__CV__'],[/اچ\s*وی\s*تی/gi,'__HVT__'],[/آی\s*بی\s*وی/gi,'__IBV__'],[/آی\s*بی\s*دی/gi,'__IBD__'],[/آی\s*ال\s*تی/gi,'__ILT__'],[/اِن\s*دی/gi,'__ND__'],[/اِی\s*آی/gi,'__AI__'],[/آر\s*ان\s*ای/gi,'__RNA__'],[/دی\s*ان\s*ای/gi,'__DNA__']];
    for(const [re,t] of protectedPairs)text=text.replace(re,t);

    // Spoken fractions/decimals are handled before integer phrases.
    text=text.replace(/(یک|دو|سه|چهار|پنج|شش|هفت|هشت|نه|ده)\s+و\s+نیم/g,(_,n)=>{const v=WORDS[n];return String(v+0.5).replace('.', '٫');});
    text=text.replace(/(صفر|یک|دو|سه|چهار|پنج|شش|هفت|هشت|نه|ده)\s+ممیز\s+(صفر|یک|دو|سه|چهار|پنج|شش|هفت|هشت|نه)/g,(_,a,b)=>`${WORDS[a]}٫${WORDS[b]}`);
    ...
    text=text.replace(numberRe,(all,before,phrase)=>{const n=parseSpokenNumber(phrase.trim().split(/\s+/));return n==null?all:before+digits(n);});

    const terms=[
      [/مایکو\s*پلا[زظ]ما/gi,'مایکوپلاسما'],[/مایکوپلاسموز/gi,'مایکوپلاسموز'],[/گامبورو/gi,'گامبورو'],[/نیوکاسل/gi,'نیوکاسل'],[/برونشیت\s+عفونی/gi,'برونشیت عفونی'],[/لارنگو\s*تراکئیت/gi,'لارنگوتراکئیت'],[/کوکسیدیوز/gi,'کوکسیدیوز'],[/کلی\s*باسیلوز/gi,'کلی‌باسیلوز'],[/اشرشیا\s*کلی/gi,'اشرشیاکلی'],[/سالمونلا/gi,'سالمونلا'],[/کمپیلوباکتر/gi,'کمپیلوباکتر'],[/کلستریدیوم/gi,'کلستریدیوم'],[/آسپرژیلوز/gi,'آسپرژیلوز'],[/سینوزیت/gi,'سینوزیت'],[/تنوسینوویت/gi,'تنوسینوویت'],[/پودو\s*درماتیت/gi,'پودودرماتیت'],[/آسیت/gi,'آسیت'],[/کیسه\s*های\s*هوایی/gi,'کیسه‌های هوایی'],[/کیسه\s+هوایی/gi,'کیسه هوایی'],[/بورس\s+فابریسیوس/gi,'بورس فابریسیوس'],[/آنتی\s*بیوگرام/gi,'آنتی‌بیوگرام'],[/آنتی\s*بادی/gi,'آنتی‌بادی'],[/آنتی\s*بیوتیک/gi,'آنتی‌بیوتیک'],[/تیتر\s+آنتی\s*بادی/gi,'تیتر آنتی‌بادی'],[/دهیدراتاسیون/gi,'دهیدراتاسیون'],[/گرمازدگی/gi,'گرمازدگی'],[/تهویه/gi,'تهویه'],[/آمونیاک/gi,'آمونیاک'],[/تراشه/gi,'تراشه'],[/سرولوژی/gi,'سرولوژی'],[/کالبد\s*گشایی/gi,'کالبدگشایی'],[/ضایعات\s+ماکروسکوپی/gi,'ضایعات ماکروسکوپی'],
      [/جوجه\s*گوشتی/gi,'جوجه گوشتی'],[/مرغ\s*گوشتی/gi,'مرغ گوشتی'],[/گوشتی/gi,'گوشتی'],[/تخمگذار/gi,'تخم‌گذار'],[/تخم\s*گذار/gi,'تخم‌گذار'],[/مرغ\s*مادر/gi,'مرغ مادر'],[/مادر\s*گوشتی/gi,'مادر گوشتی'],[/مادر\s*تخم\s*گذار/gi,'مادر تخم‌گذار'],[/پولت/gi,'پولت'],[/پولِت/gi,'پولت'],[/گله\s*مادر/gi,'گله مادر'],[/گله\s*تخم\s*گذار/gi,'گله تخم‌گذار'],[/گله\s*گوشتی/gi,'گله گوشتی'],
      [/مرگ\s*و\s*میر/gi,'مرگ‌ومیر'],[/تلفات/gi,'تلفات'],[/مصرف\s*خوراک/gi,'مصرف خوراک'],[/مصرف\s*آب/gi,'مصرف آب'],[/ضریب\s*تبدیل\s*غذایی/gi,'ضریب تبدیل غذایی'],[/یکنواختی/gi,'یکنواختی'],[/وزن\s*کشی/gi,'وزن‌کشی'],[/وزن\s*بدن/gi,'وزن بدن'],
      [/واکسن\s*نیوکاسل/gi,'واکسن نیوکاسل'],[/واکسن\s*برونشیت/gi,'واکسن برونشیت'],[/واکسن\s*گامبورو/gi,'واکسن گامبورو'],[/واکسن\s*مارک/gi,'واکسن مارک'],[/واکسن\s*آبله/gi,'واکسن آبله'],[/واکسن\s*کمپیلوباکتر/gi,'واکسن کمپیلوباکتر'],[/قطره\s*چشم/gi,'قطره چشمی'],[/اسپری/gi,'اسپری'],[/آب\s*آشامیدنی/gi,'آب آشامیدنی'],[/آبخوری/gi,'آبخوری'],[/دانخوری/gi,'دانخوری'],[/بستر/gi,'بستر'],[/رطوبت\s*بستر/gi,'رطوبت بستر'],[/تهویه\s*تونلی/gi,'تهویه تونلی'],[/تهویه\s*حداقلی/gi,'تهویه حداقلی'],
      // Layer / pullet terminology
      [/تولید\s*تخم/gi,'تولید تخم'],[/درصد\s*تولید/gi,'درصد تولید'],[/درصد\s*پیک/gi,'درصد پیک تولید'],[/پیک\s*تولید/gi,'پیک تولید'],[/تداوم\s*تولید/gi,'تداوم تولید'],[/تولید\s*روز\s*مرغ/gi,'تولید روزمرغ'],[/تولید\s*مرغ\s*روز/gi,'تولید روزمرغ'],[/مرغ\s*روز/gi,'مرغ‌روز'],[/مرغ\s*خانه/gi,'مرغ‌خانه'],[/تخم\s*شکسته/gi,'تخم شکسته'],[/تخم\s*کثیف/gi,'تخم کثیف'],[/تخم\s*کف\s*بستر/gi,'تخم کف بستر'],[/تخم\s*کفی/gi,'تخم کف بستر'],[/وزن\s*تخم/gi,'وزن تخم'],[/جرم\s*تخم/gi,'جرم تخم'],[/جرم\s*روزانه\s*تخم/gi,'جرم روزانه تخم'],[/جرم\s*تخم\s*تجمعی/gi,'جرم تجمعی تخم'],[/استحکام\s*پوسته/gi,'استحکام پوسته'],[/ضخامت\s*پوسته/gi,'ضخامت پوسته'],[/واحد\s*هاو/gi,'واحد هاو'],[/هاو\s*یونیت/gi,'واحد هاو'],[/قله\s*تولید/gi,'قله تولید'],[/بلوغ\s*جنسی/gi,'بلوغ جنسی'],[/تحریک\s*نوری/gi,'تحریک نوری'],[/فوتواستیمولیشن/gi,'فوتواستیمولیشن'],[/برنامه\s*نوری/gi,'برنامه نوری'],[/انتقال\s*به\s*تولید/gi,'انتقال به تولید'],[/دوره\s*پرورش/gi,'دوره پرورش'],[/دوره\s*تولید/gi,'دوره تولید'],[/میزان\s*دان/gi,'میزان دان'],[/مقدار\s*دان/gi,'مقدار دان'],[/دان\s*روزانه/gi,'دان روزانه'],[/مصرف\s*دان\s*روزانه/gi,'مصرف دان روزانه'],[/تبدیل\s*خوراک\s*به\s*تخم/gi,'تبدیل خوراک به تخم'],[/خوراک\s*به\s*ازای\s*دوازده\s*تخم/gi,'خوراک به ازای دوازده تخم'],[/قابلیت\s*حیات/gi,'قابلیت حیات'],[/زیستایی/gi,'زیستایی'],
      // Breeder / parent-stock terminology
      [/گله\s*والد/gi,'گله والد'],[/والدین/gi,'والدین'],[/مادر\s*والد/gi,'مرغ مادر'],[/خروس\s*مادر/gi,'خروس مادر'],[/نسبت\s*خروس\s*به\s*ماده/gi,'نسبت خروس به ماده'],[/نسبت\s*نر\s*به\s*ماده/gi,'نسبت نر به ماده'],[/باروری/gi,'باروری'],[/نطفه\s*داری/gi,'نطفه‌داری'],[/قابلیت\s*جوجه\s*در\s*آوری/gi,'قابلیت جوجه‌درآوری'],[/جوجه\s*در\s*آوری/gi,'جوجه‌درآوری'],[/درصد\s*هچ/gi,'درصد هچ'],[/هچری/gi,'هچری'],[/تخم\s*قابل\s*جوجه\s*کشی/gi,'تخم قابل جوجه‌کشی'],[/تخم\s*قابل\s*جوجه\s*در\s*آوری/gi,'تخم قابل جوجه‌درآوری'],[/تخم\s*قابل\s*ستر/gi,'تخم قابل ستر'],[/تخم\s*ستر/gi,'تخم ستر'],[/ذخیره\s*تخم/gi,'ذخیره تخم'],[/نگهداری\s*تخم/gi,'نگهداری تخم'],[/جمع\s*آوری\s*تخم/gi,'جمع‌آوری تخم'],[/آشیانه/gi,'آشیانه'],[/آشیانه\s*دستی/gi,'آشیانه دستی'],[/آشیانه\s*اتوماتیک/gi,'آشیانه اتوماتیک'],[/آموزش\s*آشیانه/gi,'آموزش آشیانه'],[/تخم\s*کف\s*خانه/gi,'تخم کف خانه'],[/پرورش\s*مادر/gi,'پرورش مادر'],[/پرورش\s*والد/gi,'پرورش والد'],[/گریدینگ/gi,'گریدینگ'],[/درجه\s*بندی/gi,'درجه‌بندی'],[/وزن\s*کشی\s*انفرادی/gi,'وزن‌کشی انفرادی'],[/وزن\s*کشی\s*توده\s*ای/gi,'وزن‌کشی توده‌ای'],[/پر کردن\s*چینه\s*دان/gi,'پر کردن چینه‌دان'],[/چینه\s*دان/gi,'چینه‌دان'],[/فاصله\s*استخوان\s*های\s*شرمگاهی/gi,'فاصله استخوان‌های شرمگاهی'],[/فاصله\s*استخوان\s*های\s*پین/gi,'فاصله استخوان‌های پین'],[/تغذیه\s*جداگانه\s*جنسی/gi,'تغذیه جداگانه جنسی'],[/محدودیت\s*خوراک/gi,'محدودیت خوراک'],[/خوراک\s*محدود/gi,'خوراک محدود'],[/اسپایکینگ/gi,'اسپایکینگ'],[/اسپایک/gi,'اسپایک'],[/جفت\s*گیری/gi,'جفت‌گیری'],[/نر\s*ماده/gi,'نر و ماده'],
      // Rearing / welfare / health vocabulary
      [/تنظیم\s*حرارت/gi,'تنظیم حرارت'],[/دمای\s*محیط/gi,'دمای محیط'],[/رطوبت\s*نسبی/gi,'رطوبت نسبی'],[/کیفیت\s*بستر/gi,'کیفیت بستر'],[/آب\s*و\s*خوراک/gi,'آب و خوراک'],[/نسبت\s*آب\s*به\s*خوراک/gi,'نسبت آب به خوراک'],[/سلامت\s*پا/gi,'سلامت پا'],[/زخم\s*کف\s*پا/gi,'زخم کف پا'],[/درماتیت\s*کف\s*پا/gi,'درماتیت کف پا'],[/آسیب\s*مفصل/gi,'آسیب مفصل'],[/کیفیت\s*پوسته/gi,'کیفیت پوسته']
    ];
    for(const [re,rep] of terms)text=text.replace(re,rep);
    return digits(text).replace(/__PCR__/g,'PCR').replace(/__ELISA__/g,'ELISA').replace(/__FCR__/g,'FCR').replace(/__CV__/g,'CV').replace(/__QPCR__/g,'qPCR').replace(/__RTPCR__/g,'RT-PCR').replace(/__HVT__/g,'HVT').replace(/__IBV__/g,'IBV').replace(/__IBD__/g,'IBD').replace(/__ILT__/g,'ILT').replace(/__AI__/g,'AI').replace(/__ND__/g,'ND').replace(/__RNA__/g,'RNA').replace(/__DNA__/g,'DNA');
  }

  function eligible(el){if(!el||el.nodeType!==1||el.disabled||el.readOnly||el.hidden)return false;if(el.dataset.voiceDisabled==='true'||el.hasAttribute('data-no-voice')||el.closest('[data-voice-disabled="true"],.no-voice'))return false;if(el.tagName==='TEXTAREA')return true;if(el.tagName!=='INPUT')return false;const type=(el.getAttribute('type')||'text').toLowerCase();if(type!=='text'||NUMERIC_MODE.test(el.getAttribute('inputmode')||'')||el.classList.contains('jalali-input'))return false;return !DENY.test((el.id||'')+' '+(el.name||''));}
  function label(el){if(el.id&&window.CSS&&CSS.escape){const l=document.querySelector('label[for="'+CSS.escape(el.id)+'"]');if(l)return l.textContent.trim();}const p=el.closest('label');return p?p.textContent.trim():'متن';}
  ...
  function tokenTable(raw){if(Array.isArray(raw))return raw;if(raw&&Array.isArray(raw.tokens))return raw.tokens;if(raw&&raw.vocab&&typeof raw.vocab==='object')return Object.entries(raw.vocab).sort((a,b)=>a[1]-b[1]).map(x=>x[0]);if(raw&&typeof raw==='object'){const e=Object.entries(raw).filter(x=>/^\d+$/.test(String(x[1]))).sort((a,b)=>Number(a[1])-Number(b[1]));if(e.length)return e.map(x=>x[0]);}throw new Error('TOKENS_INVALID');}
  ...
  // Exact Shenava/NeMo center=true contract: reflect-pad 256 samples before framing.
  function makeFeatures(samples,filters){
    ...
    const pre=new Float32Array(x.length);let prev=0;for(let i=0;i<x.length;i++){const v=x[i];pre[i]=v-0.97*prev;prev=v;}
    const padded=new Float32Array(pre.length+CFG.centerPad*2);for(let i=0;i<padded.length;i++)padded[i]=pre[reflectIndex(i-CFG.centerPad,pre.length)];
    const frames=Math.max(1,Math.min(CFG.fixedFrames,Math.floor(pre.length/CFG.hopLength)+1));
    const out=new Float32Array(CFG.nMels*CFG.fixedFrames);const win=new Float64Array(CFG.winLength);for(let i=0;i<CFG.winLength;i++)win[i]=0.5-0.5*Math.cos(2*Math.PI*i/(CFG.winLength-1));
    for(let f=0;f<frames;f++){const start=f*CFG.hopLength,buf=new Float64Array(CFG.nFft);for(let j=0;j<CFG.winLength;j++)buf[j]=(padded[start+j]||0)*win[j];const{re,im}=fftReal(buf,CFG.nFft);const power=new Float64Array(CFG.nFft/2+1);for(let k=0;k<power.length;k++)power[k]=re[k]*re[k]+im[k]*im[k];for(let m=0;m<CFG.nMels;m++){const row=filters[m]||[];let sum=0;for(let k=0;k<row.length&&k<power.length;k++)sum+=Number(row[k]||0)*power[k];out[m*CFG.fixedFrames+f]=Math.log(sum+5.960464477539063e-8);}}
    return{data:out,frames};
  }
  function ctcDecode(logits,lengths,tokens){const steps=Math.min(Number(lengths[0]||logits.dims[1]),logits.dims[1]),vocab=logits.dims[2];let prev=-1,out='';for(let t=0;t<steps;t++){let best=0,bestVal=-Infinity;for(let v=0;v<vocab;v++){const val=logits.data[t*vocab+v];if(val>bestVal){bestVal=val;best=v;}}if(best!==CFG.blankId&&best!==prev){const tok=tokens[best]==null?'':String(tokens[best]);if(!(tok.startsWith('<')&&tok.endsWith('>')))out+=tok;}prev=best;}return out.replace(/▁/g,' ').replace(/\s+/g,' ').trim();}
  async function initEngine(){let ort;try{ort=await loadScript(CFG.ortCdn);}catch(_){ort=await loadScript(CFG.ortCdnFallback);}ort.env.wasm.numThreads=1;ort.env.wasm.simd=true;const[pre,mel,tok]=await Promise.all([json(asset(CFG.preFile)),json(asset(CFG.melFile)),json(asset(CFG.tokensFile))]);const filters=melFilters(mel),tokens=tokenTable(tok),providers=[];if(navigator.gpu)providers.push('webgpu');providers.push('wasm');const session=await ort.InferenceSession.create(asset(CFG.modelFile),{executionProviders:providers,graphOptimizationLevel:'all'});return{ort,session,filters,tokens,pre,providers};}
  ...
  async function decodeBlob(blob){if(!blob||!blob.size)throw new Error('EMPTY_AUDIO');const AC=window.AudioContext||window.webkitAudioContext;if(!AC)throw new Error('AUDIO_DECODE_UNAVAILABLE');const ctx=new AC();try{const ab=await blob.arrayBuffer(),audio=await ctx.decodeAudioData(ab.slice(0));const mono=new Float32Array(audio.length);for(let c=0;c<audio.numberOfChannels;c++){const ch=audio.getChannelData(c);for(let i=0;i<ch.length;i++)mono[i]+=ch[i]/audio.numberOfChannels;}const target=Math.max(1,Math.round(mono.length*CFG.sampleRate/audio.sampleRate)),rs=new Float32Array(target);for(let i=0;i<target;i++){const p=i*(mono.length-1)/Math.max(1,target-1),a=Math.floor(p),b=Math.min(a+1,mono.length-1),f=p-a;rs[i]=mono[a]*(1-f)+mono[b]*f;}const e=await engine(),feat=makeFeatures(rs,e.filters),tensor=new e.ort.Tensor('float16',float16Data(feat.data),[1,CFG.nMels,CFG.fixedFrames]),len=new e.ort.Tensor('int64',BigInt64Array.from([BigInt(feat.frames)]),[1]);const out=await e.session.run({processed_signal:tensor,processed_signal_length:len});return normalizeTerms(ctcDecode(out.logits,out.encoded_lengths,e.tokens));}finally{try{await ctx.close();}catch(_){}}}
