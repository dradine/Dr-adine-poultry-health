...
/* ADINE - Weekly sampling runtime repair.
   The previous sampling wrapper lived in weekly-completion-restore-v1.js
   and referenced helper functions that were scoped inside another IIFE.
   That caused calculateWeightStatistics() to throw at runtime, so the
   results panel never rendered. This layer unwraps that wrapper and changes
   only recommendedSampleSize/samplingStatus for broiler weekly monitoring.
   CV, SD, mean, uniformity, CI, min/max and all persisted calculations are
   returned unchanged from the original engine. */
(function(){'use strict';
  ...
  function activeFlock(){
    try{if(typeof currentFlock!=='undefined'&&currentFlock)return currentFlock}catch(e){}
    return window.currentFlockForSpecialized||window.currentFlock||null;
  }
  function isBroiler(f){
    const t=String(f?.production_type||f?.productionType||'').trim().toLowerCase();
    return /گوشتی|broiler|meat/.test(t);
  }
  function population(){
    const live=numberValue(document.getElementById('liveBirds')?.value);
    if(Number.isFinite(live)&&live>0)return Math.floor(live);
    const f=activeFlock();
    if(!f)return null;
    const fields=[f.initial_bird_count,f.initialBirdCount,f.initial_birds,f.placement_birds,f.bird_count];
    for(const v of fields){const x=numberValue(v);if(Number.isFinite(x)&&x>0)return Math.floor(x)}
    return null;
  }
  function repair(){
    const current=window.calculateWeightStatistics;
    if(typeof current!=='function')return false;
    if(current.__populationSamplingRuntimeV2)return true;
    const original=typeof current.__original==='function'?current.__original:current;
    function repaired(weights){
      const result=original.apply(this,arguments);
      const f=activeFlock();
      if(!result||typeof result!=='object'||!isBroiler(f))return result;
      const p=population();
      const recommended=Number.isFinite(p)&&p>0?Math.max(100,Math.ceil(p*0.01)):100;
    ...
    repaired.__populationSamplingRuntimeV2=true;
    repaired.__original=original;
    window.calculateWeightStatistics=repaired;
    return true;
  }
  function start(){
    if(repair())return;
    let i=0;const t=setInterval(()=>{if(repair()||++i>120)clearInterval(t)},100);
  }
