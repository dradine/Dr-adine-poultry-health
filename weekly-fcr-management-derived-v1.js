...
(function(g){
  ...
  async function load(){
    ...
    standards=data.map(x=>({week:num(x.week_number),age:num(x.age_days),value:num(x.weekly_fcr),cum:num(x.cumulative_fcr),source:x.source_name,year:x.source_year,method:x.method})).filter(x=>x.week!=null&&x.value!=null);
  ...
  function patchV2(){
    ...
    const old=weekly.querySelector("small"); if(old)old.textContent=`استاندارد مدیریتی FCR هفتگی: ${fmt(t.value)} | ${t.source||"Performance Objectives"} (${t.year||"—"})`;
    weekly.dataset.managementFcr=String(t.value);
    let cum=findKpiByLabel("FCR تجمعی");
    if(cum){const ref=cum.querySelector("small");if(ref)ref.textContent=`استاندارد رسمی FCR تجمعی: ${fmt(t.cum)} | ${t.source||"Performance Objectives"} (${t.year||"—"})`;cum.dataset.officialCumulativeFcr=String(t.cum);}
    ...
    add(weekly,"weekly","استاندارد مدیریتی FCR هفتگی",t.value,`${t.source||"Performance Objectives"} (${t.year||"—"})`);
    if(cum)add(cum,"cumulative","استاندارد رسمی FCR تجمعی",t.cum,`${t.source||"Performance Objectives"} (${t.year||"—"})`);
    return true;
  }
  function patchLegacy(){
    ...
    const t=targetForWeek(w); if(!t)return false;
    const ref=wkMetric.querySelector(".ref"); if(ref)ref.textContent=`استاندارد مدیریتی FCR هفتگی: ${fmt(t.value)} | ${t.source||"Performance Objectives"} (${t.year||"—"})`;
    if(cumMetric){const cref=cumMetric.querySelector(".ref");if(cref)cref.textContent=`استاندارد رسمی FCR تجمعی: ${fmt(t.cum)} | ${t.source||"Performance Objectives"} (${t.year||"—"})`;}
    return true;
  }
  function patchCharts(){
    ...
    Object.values(instances).forEach(ch=>{try{
      ...
      if(!official)return;
      official.label="استاندارد رسمی تجمعی"; official.data=cumulative;
