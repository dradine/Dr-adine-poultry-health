/* ADINE POULTRY HEALTH — CANONICAL BROILER FCR REPORT ADAPTER V5 */
"use strict";
(function(global){
  ...
  function removeManagementCard(){
    [...document.querySelectorAll('.metric')].filter(el=>String(el.querySelector('.label')?.textContent||'').trim()==='FCR مدیریتی').forEach(el=>el.remove());
  }
  ...
  function patch(){removeManagementCard();const r=currentRow();if(!r)return;setMetric('FCR هفتگی',r.weekly_fcr,`هدف مدیریتی: ${fmt(r.management_weekly_fcr)}`);setMetric('FCR تجمعی',r.cumulative_fcr,`استاندارد رسمی تجمعی: ${fmt(r.official_cumulative_fcr)}${r.official_source?' — '+r.official_source:''}`);patchCharts(r.week_number);}
