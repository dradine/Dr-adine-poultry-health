...
function weeklySpecializedType(flock){
  const raw=String(flock?.production_type||flock?.productionType||"broiler").trim().toLowerCase();
...
function renderWeeklySpecializedFields(flock){
  ...
  const fields=[...WEEKLY_SPECIALIZED_FIELDS.common,...(WEEKLY_SPECIALIZED_FIELDS[type]||WEEKLY_SPECIALIZED_FIELDS.broiler)];
  const labels={broiler:"گوشتی: رشد، یکنواختی، FCR، دان/آب و کنترل محیط.",pullet:"پولت: کنترل منحنی وزن، یکنواختی و آماده‌سازی تولید.",layer:"تخم‌گذار: تولید، وزن تخم، مصرف دان و کیفیت تخم.",breeder:"مادر: وزن، یکنواختی، تولید و کیفیت تخم قابل جوجه‌کشی."};
  if(intro) intro.innerHTML=`<strong>${labels[type]||labels.broiler}</strong><div class="weekly-special-note">شاخص‌های ستاره‌دار، شاخص‌های اصلی ارزیابی هستند.</div>`;
...
function getWeeklySpecializedMetrics(){const out={};document.querySelectorAll("[data-weekly-specialized]").forEach(input=>{const key=input.dataset.weeklySpecialized;const raw=String(input.value||"").trim();if(!raw)return;if(input.inputMode==="decimal"){const n=Number(normalizeWeeklyDigits(raw));if(Number.isFinite(n))out[key]=n;}else out[key]=raw;});return out;}
...
function calculateWeeklySpecializedDerived(metrics,flock,previousRecords){const out={...(metrics||{})};const type=weeklySpecializedType(flock);const rows=Array.isArray(previousRecords)?previousRecords:[];const live=Number(out.live_birds||flock?.current_bird_count||flock?.live_birds||0);if((type==='layer'||type==='breeder')&&out.egg_mass_kg==null){const eggs=Number(out.egg_count??out.female_egg_count??0),ew=Number(out.egg_weight_g||0);if(eggs>0&&ew>0)out.egg_mass_kg=Number((eggs*ew/1000).toFixed(3));}if((type==='layer'||type==='breeder')&&out.hen_day_pct==null&&live>0){const eggs=Number(out.egg_count??out.female_egg_count??0);if(eggs>0)out.hen_day_pct=Number((eggs/(live*7)*100).toFixed(3));}if(type==='layer'&&out.hen_housed_pct==null&&live>0){const eggs=Number(out.egg_count||0);if(eggs>0)out.hen_housed_pct=Number((eggs/(live*7)*100).toFixed(3));}if(out.water_feed_ratio==null){const water=Number(flock?._weekly_water_total||0),feed=Number(flock?._weekly_feed_total||0);if(water>0&&feed>0)out.water_feed_ratio=Number((water/feed).toFixed(3));}if(out.weekly_gain_g==null&&rows.length){const last=[...rows].sort((a,b)=>Number(a.age_days||a.ageDays||0)-Number(b.age_days||b.ageDays||0)).at(-1);const w=Number(out.measured_weight_g||out.average_weight_g||0),pw=Number(last?.average_weight_g||last?.averageWeight||0);if(w>0&&pw>0)out.weekly_gain_g=Number((w-pw).toFixed(1));}return out;}
function validateSpecializedMetrics(type,metrics){const percentKeys=["hen_day_pct","hen_housed_pct","dirty_eggs_pct","cracked_eggs_pct","floor_eggs_pct","hatching_egg_pct","fertility_pct","hatchability_pct","male_female_ratio","female_uniformity_pct","pelvic_fat_pct"];for(const k of percentKeys){if(metrics[k]!=null&&(metrics[k]<0||metrics[k]>100))return `${weeklyFieldLabel(k)} باید بین ۰ تا ۱۰۰ باشد.`;}if(metrics.egg_weight_g!=null&&metrics.egg_weight_g<=0)return "وزن تخم باید بزرگ‌تر از صفر باشد.";if(metrics.egg_mass_kg!=null&&metrics.egg_mass_kg<0)return "Egg Mass نمی‌تواند منفی باشد.";if(metrics.litter_score!=null&&(metrics.litter_score<0||metrics.litter_score>5))return "امتیاز بستر باید بین ۰ تا ۵ باشد.";if(metrics.footpad_score!=null&&(metrics.footpad_score<0||metrics.footpad_score>2))return "امتیاز کف پا باید بین ۰ تا ۲ باشد.";return null;}
function calculateWeeklyCumulativeConversion(records,current,type){const rows=Array.isArray(records)?[...records]:[];const currentFeed=Number(current?.feed_total_kg||0);if(currentFeed<=0)return null;const all=[...rows.filter(r=>String(r.id)!==String(current?.id)),current].sort((a,b)=>Number(a.week_number)-Number(b.week_number));const t=weeklySpecializedType({production_type:type});const first=all.find(r=>Number(r.average_weight_g)>0);if(!first)return null;const feed=all.reduce((sum,r)=>sum+Number(r.feed_total_kg||0),0);if(t==="layer"||t==="breeder"){const eggMass=all.reduce((sum,r)=>sum+Number(r.production_metrics?.egg_mass_kg||0),0);return eggMass>0?Number((feed/eggMass).toFixed(3)):null;}const last=all[all.length-1];const birds0=Number(first.live_birds||0),birdsn=Number(last.live_birds||0);const w0=Number(first.average_weight_g||0),wn=Number(last.average_weight_g||0);if(!birds0||!birdsn||!w0||!wn)return null;const gainKg=(birdsn*wn-birds0*w0)/1000;return gainKg>0?Number((feed/gainKg).toFixed(3)):null;}
...
window.WEEKLY_SPECIALIZED_FIELDS=WEEKLY_SPECIALIZED_FIELDS;window.getWeeklySpecializedMetrics=getWeeklySpecializedMetrics;window.loadWeeklySpecializedMetrics=loadWeeklySpecializedMetrics;window.clearWeeklySpecializedMetrics=clearWeeklySpecializedMetrics;window.calculateWeeklyCumulativeConversion=calculateWeeklyCumulativeConversion;window.calculateWeeklySpecializedDerived=calculateWeeklySpecializedDerived;window.validateSpecializedMetrics=validateSpecializedMetrics;window.closeCurrentFlockPeriod=closeCurrentFlockPeriod;window.toggleWeeklyAdvanced=toggleWeeklyAdvanced;
document.addEventListener("DOMContentLoaded",()=>{setTimeout(()=>{const flock=window.currentFlockForSpecialized||window.currentFlock;if(flock)renderWeeklySpecializedFields(flock);else{const card=document.getElementById("specializedMetricsCard");if(card)card.style.display="none";}},150);});
